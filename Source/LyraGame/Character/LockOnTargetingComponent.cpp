...
ULockOnTargetingComponent::ULockOnTargetingComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	PrimaryComponentTick.bStartWithTickEnabled = false;
}

void ULockOnTargetingComponent::BeginPlay()
{
	Super::BeginPlay();
...
void ULockOnTargetingComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void ULockOnTargetingComponent::ToggleLockOn()
{
	if (bIsLockedOn)
	{
		DisableLockOn();
...
void ULockOnTargetingComponent::EnableLockOn()
{
	if (bIsLockedOn)
	{
		return;
...
void ULockOnTargetingComponent::DisableLockOn()
{
	if (!bIsLockedOn)
	{
		return;
...
void ULockOnTargetingComponent::SwitchTargetRight()
{
	if (!bIsLockedOn)
	{
		return;
...
void ULockOnTargetingComponent::SwitchTargetLeft()
{
	if (!bIsLockedOn)
	{
		return;
...
AActor* ULockOnTargetingComponent::FindNearestTarget() const
{
	if (!OwnerCharacter.IsValid())
	{
		return nullptr;
...
TArray<AActor*> ULockOnTargetingComponent::FindValidTargets() const
{
...
bool ULockOnTargetingComponent::IsValidTarget(AActor* Target) const
{
...
void ULockOnTargetingComponent::UpdateCameraToTarget(float DeltaTime)
{
...
void ULockOnTargetingComponent::ValidateCurrentTarget()
{
...
void ULockOnTargetingComponent::SwitchTarget(bool bSwitchRight)
{
	if (!CurrentTarget.IsValid() || !OwnerCharacter.IsValid())
	{
		return;
...
float ULockOnTargetingComponent::GetAngleToTarget(AActor* Target) const
{
	if (!Target || !OwnerCharacter.IsValid())
	{
		return 0.0f;
