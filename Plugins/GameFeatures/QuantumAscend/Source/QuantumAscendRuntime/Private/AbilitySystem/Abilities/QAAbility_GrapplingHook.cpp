...
#include "AbilitySystem/Abilities/QAAbility_GrapplingHook.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(QAAbility_GrapplingHook)

UQAAbility_GrapplingHook::UQAAbility_GrapplingHook(const FObjectInitializer& ObjectInitializer /*= FObjectInitializer::Get()*/)
{
	InstancingPolicy = EGameplayAbilityInstancingPolicy::InstancedPerActor;
	NetExecutionPolicy = EGameplayAbilityNetExecutionPolicy::LocalPredicted;
}

bool UQAAbility_GrapplingHook::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	if (ActorInfo && ActorInfo->AvatarActor.IsValid())
	{
		...
		if (AbilityCharacter)
		{
			UBaseCharacterMovementComponent* AbilityMovementComponent =
				Cast<UBaseCharacterMovementComponent>(AbilityCharacter->GetCharacterMovement());
...
void UQAAbility_GrapplingHook::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	if (OwnerPlayerController && OwnerCharacter && OwnerMovementComponent && IsLocallyControlled())
	{
		...
		if (OwnerPlayerController->GetHitResultUnderCursor(ECC_Visibility, true, HitResult))
		{
			...
			{
				LyraIC->BindNativeAction(InputConfig, QAGameplayTags::Ability_GrapplingHook_Pulling, ETriggerEvent::Started, this, &UQAAbility_GrapplingHook::StartMoveToTarget, /*bLogIfNotFound=*/ false);
				LyraIC->BindNativeAction(InputConfig, QAGameplayTags::Ability_GrapplingHook_Cancel, ETriggerEvent::Completed, this, &UQAAbility_GrapplingHook::CancelGrapplingHook, /*bLogIfNotFound=*/ false);
			}
			...
			if (TickTask)
			{
				TickTask->OnTick.AddDynamic(this, &UQAAbility_GrapplingHook::TickAbility);
...
void UQAAbility_GrapplingHook::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
	Super::EndAbility(Handle, ActorInfo, ActivationInfo, bReplicateEndAbility, bWasCancelled);
}

void UQAAbility_GrapplingHook::TickAbility(float DeltaTime)
{
	SwingMovement(GrapplingTargetLocation);
}

bool UQAAbility_GrapplingHook::PerformHookTrace(FVector TargetPos)
{
	UWorld* World = GetWorld();
...
bool UQAAbility_GrapplingHook::PerformTeleportTrace(FVector TargetPos, FVector& AvailablePos)
{
	UWorld* World = GetWorld();
...
void UQAAbility_GrapplingHook::ShootGrapplingHook(FVector TargetPos)
{
	if (OwnerCharacter)
	{
...
void UQAAbility_GrapplingHook::ServerShootGrapplingHook_Implementation(FVector TargetPos)
{
	UWorld* World = GetWorld();
	...
	if (World && OwnerCharacter && RopeActorClass && HookActorClass)
	{
		...
		if (CachedHookActor)
		{
			...
			CachedHookActor->OnHookArrived.AddDynamic(this, &UQAAbility_GrapplingHook::OnHookArrivedHandler);
		}
...
void UQAAbility_GrapplingHook::ReleaseGrpplingHook()
{
	ServerReleaseGrapplingHook();
}

void UQAAbility_GrapplingHook::ServerReleaseGrapplingHook_Implementation()
{
	UWorld* World = GetWorld();
...
void UQAAbility_GrapplingHook::CancelGrapplingHook()
{
...
void UQAAbility_GrapplingHook::OnHookArrivedHandler()
{
	Step = EGrappleStep::Ready;
}

void UQAAbility_GrapplingHook::SwingMovement(FVector TargetPos)
{
...
void UQAAbility_GrapplingHook::StartMoveToTarget()
{
