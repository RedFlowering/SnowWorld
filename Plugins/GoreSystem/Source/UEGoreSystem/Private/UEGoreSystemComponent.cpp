	...
	for (int32 Idx = 0; Idx < Bones.Num(); Idx++) {
		...
		if (NewStruct.DetachedMesh && GoreSettings.GlobalAnimInstance) {
			...
			for (int32 i = 0; i < AllComp.Num(); i++) {
				...
				}else if (AllComp[i] && NiagaraComp != AllComp[i] && AllComp[i] != CParticleSyst && (CSKComp->BoneIsChildOf(CSKComp->GetSocketBoneName(AllComp[i]->GetAttachSocketName()), CSKComp->GetParentBone(Bones[Idx].BoneName)))) {
