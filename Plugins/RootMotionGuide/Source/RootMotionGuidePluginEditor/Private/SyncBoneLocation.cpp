...
#define LOCTEXT_NAMESPACE "SyncBoneLocation"

FReply USyncBoneLocation::SyncBoneLocation()
{
	FName IKBoneName = SyncBoneName;		/** Name of bone to control. This is the main bone chain to modify from. **/
	bool bAllowStretching = false;			/** Should stretching be allowed, to be prevent over extension */
	float StartStretchRatio = 1.0f;			/** Limits to use if stretching is allowed. This value determines when to start stretch. For example, 0.9 means once it reaches 90% of the whole length of the limb, it will start apply. */
	float MaxStretchScale = 1.2f;			/** Limits to use if stretching is allowed. This value determins what is the max stretch scale. For example, 1.5 means it will stretch until 150 % of the whole length of the limb.*/
	...
	if (AnimSequence)
	{
		...
		if (SourceAnimSequence == nullptr)
		{
			Notification(LOCTEXT("No Source AnimSequence", "No Source AnimSequence"), 8.0f, SNotificationItem::CS_Fail);
			return FReply::Handled();
		}

		USkeleton* SrcSkeleton = SourceAnimSequence->GetSkeleton();
		const FReferenceSkeleton& SrcRefSkeleton = SrcSkeleton->GetReferenceSkeleton();
		int32 SrcNumBones = SrcRefSkeleton.GetNum();

		int32 SourceBoneIndex = SrcRefSkeleton.FindBoneIndex(SourceBoneName);
		if (SourceBoneIndex == INDEX_NONE)
		{
			Notification(LOCTEXT("Invalid Source Bone", "Invalid Source Bone"), 8.0f, SNotificationItem::CS_Fail);
			return FReply::Handled();
		}
		...
		TArray< TArray< FTransform > > SrcAnimationDataInCS;
		SrcAnimationDataInCS.Empty(SrcNumBones);
		SrcAnimationDataInCS.AddZeroed(SrcNumBones);

		for (int32 BoneIndex = 0; BoneIndex < SrcNumBones; BoneIndex++)
		{
			SrcAnimationDataInCS[BoneIndex].Empty(NumKeys);
			SrcAnimationDataInCS[BoneIndex].AddUninitialized(NumKeys);

			FName BoneName = SrcRefSkeleton.GetBoneName(BoneIndex);
			int32 ParentBoneIndex = SrcRefSkeleton.GetParentIndex(BoneIndex);

			EBoneTranslationRetargetingMode::Type RetargetMode = SrcSkeleton->GetBoneTranslationRetargetingMode(BoneIndex);

			if (SourceAnimSequence->GetDataModel()->IsValidBoneTrackName(BoneName))
			{
				for (int32 Key = 0; Key < NumKeys; Key++)
				{
					int32 KeyIndex1, KeyIndex2;
					float SubFrame;
					FAnimationRuntime::GetKeyIndicesFromTime(KeyIndex1, KeyIndex2, SubFrame, Key * IntervalTime, SourceAnimSequence->GetDataModel()->GetFrameRate(), SourceAnimSequence->GetDataModel()->GetNumberOfKeys());

					FTransform AnimatedLocalKey = SourceAnimSequence->GetDataModel()->EvaluateBoneTrackTransform(BoneName, FFrameTime(KeyIndex1, SubFrame), SourceAnimSequence->Interpolation);
					

					if (RetargetMode == EBoneTranslationRetargetingMode::Skeleton)
					{
						AnimatedLocalKey.SetLocation(SrcRefSkeleton.GetRefBonePose()[BoneIndex].GetLocation());
					}

					if (ParentBoneIndex != INDEX_NONE)
					{
						SrcAnimationDataInCS[BoneIndex][Key] = AnimatedLocalKey * SrcAnimationDataInCS[ParentBoneIndex][Key];
					}
					else
					{
						SrcAnimationDataInCS[BoneIndex][Key] = AnimatedLocalKey;
					}
				}
			}
			else
			{
				FTransform LocalTransform = SrcRefSkeleton.GetRefBonePose()[BoneIndex];

				for (int32 Key = 0; Key < NumKeys; Key++)
				{
					if (ParentBoneIndex != INDEX_NONE)
					{
						SrcAnimationDataInCS[BoneIndex][Key] = LocalTransform * SrcAnimationDataInCS[ParentBoneIndex][Key];
					}
					else
					{
						SrcAnimationDataInCS[BoneIndex][Key] = LocalTransform;
					}
				}
			}
		}
		...
		{
			FScopedTransaction Transation(LOCTEXT("SyncBoneLocation", "SyncBoneLocation"));
			AnimSequence->Modify();
			AnimSequence->GetController().OpenBracket(FText::FromString(TEXT("SyncBoneLocation")));
			...
			for (int32 Key = 0; Key < NumKeys; Key++)
			{
				...
				if (Key >= StartFrame && Key <= (EndFrame + BlendOutFrames))
				{
					...
					EffectorTransform.SetLocation(SrcAnimationDataInCS[SourceBoneIndex][Key].GetLocation());

					FVector JointTargetPos = GetJointTargetPosition(SrcRefSkeleton, SrcAnimationDataInCS, SourceBoneIndex, RefSkeleton, AnimationDataInCS, IKBoneIndex, Key);
...
void USyncBoneLocation::ConvertBoneSpaceTransformToCS(const FReferenceSkeleton& RefSkeleton, TArray<TArray<FTransform>> AnimationDataInCS, FTransform & InOutBoneSpaceTM, int32 BoneIndex, int32 Key, EBoneControlSpace Space)
{
	switch (Space)
	{
	case BCS_WorldSpace:
...
void USyncBoneLocation::RemoveTwist(const FTransform & InParentTransform, FTransform & InOutTransform, const FTransform & OriginalLocalTransform, const FVector & InAlignVector)
{
...
FVector USyncBoneLocation::GetJointTargetPosition(const FReferenceSkeleton & SrcRefSkeleton, TArray<TArray<FTransform>> SrcAnimationDataInCS, int32 SrcEndBoneIndex, const FReferenceSkeleton & RefSkeleton, TArray<TArray<FTransform>> AnimationDataInCS, int32 EndBoneIndex, int32 Key)
{
	int32 LowerLimbIndex = RefSkeleton.GetParentIndex(EndBoneIndex);
	int32 UpperLimbIndex = RefSkeleton.GetParentIndex(LowerLimbIndex);

	FVector EndBonePos = AnimationDataInCS[EndBoneIndex][Key].GetLocation();
	FVector LowerLimbPos = AnimationDataInCS[LowerLimbIndex][Key].GetLocation();
	FVector UpperLimbPos = AnimationDataInCS[UpperLimbIndex][Key].GetLocation();

	FVector JointTargetPos = LowerLimbPos;

	bool bInvalidLimb = false;
	int32 SrcLowerLimbIndex = SrcRefSkeleton.GetParentIndex(SrcEndBoneIndex);
	int32 SrcUpperLimbIndex = INDEX_NONE;

	if (SrcLowerLimbIndex != INDEX_NONE)
	{
		SrcUpperLimbIndex = SrcRefSkeleton.GetParentIndex(SrcLowerLimbIndex);
	}

	if (SrcLowerLimbIndex == INDEX_NONE || SrcUpperLimbIndex == INDEX_NONE)
	{
		bInvalidLimb = true;
	}

	if (bInvalidLimb == false)
	{
		FVector SrcEndBonePos = SrcAnimationDataInCS[SrcEndBoneIndex][Key].GetLocation();
		FVector SrcLowerLimbPos = SrcAnimationDataInCS[SrcLowerLimbIndex][Key].GetLocation();
		FVector SrcUpperLimbPos = SrcAnimationDataInCS[SrcUpperLimbIndex][Key].GetLocation();

		FVector SrcJointOffset = FVector::ZeroVector;

		{
			float LengthOfUpperToLower = (SrcLowerLimbPos - SrcUpperLimbPos).Size();
			float LengthOfLowerToEnd = (SrcEndBonePos - SrcLowerLimbPos).Size();

			FVector JointBasePos = (SrcEndBonePos - SrcUpperLimbPos) * (LengthOfUpperToLower / (LengthOfUpperToLower + LengthOfLowerToEnd)) + SrcUpperLimbPos;
			SrcJointOffset = SrcLowerLimbPos - JointBasePos;
		}

		{
			float LengthOfUpperToLower = (LowerLimbPos - UpperLimbPos).Size();
			float LengthOfLowerToEnd = (EndBonePos - LowerLimbPos).Size();

			FVector JointBasePos = (SrcEndBonePos - UpperLimbPos) * (LengthOfUpperToLower / (LengthOfUpperToLower + LengthOfLowerToEnd)) + UpperLimbPos;

			JointTargetPos = JointBasePos + SrcJointOffset;
		}
	}

	return JointTargetPos;
}
