...
#define LOCTEXT_NAMESPACE "AdjustAnimationTrack"


FReply UAdjustAnimationTrack::AdjustAnimationTrack()
{
	if (AnimSequence)
	{
		...
		// Adjust
		{
			...
			for (int32 key = 0; key < NumKeys; key++)
			{
				...
				AnimatedLocalKey.AddToTranslation(AdjustLocation);

				if (AdjustRotation.IsNearlyZero() == false)
				{
					AnimatedLocalKey.SetRotation(AdjustRotation.Quaternion() * AnimatedLocalKey.GetRotation());
				}
