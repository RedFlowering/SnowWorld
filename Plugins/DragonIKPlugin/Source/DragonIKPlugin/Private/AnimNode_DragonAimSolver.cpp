...
//	if(owning_skel->GetWorld()->IsGameWorld())
	{
		...
		{
			FTransform original_head_transform = MeshBases.GetComponentSpaceTransform(HeadTransforms[i].BoneIndex);
			FTransform current_head_transform = HeadTransforms[i].Transform;

			HeadTransforms[i].Transform = UKismetMathLibrary::TLerp(original_head_transform, current_head_transform, toggle_alpha);
		...
		for (int i = 0; i < LegIK_Transforms.Num(); i++)
		{
			...
			FTransform original_leg_transform = MeshBases.GetComponentSpaceTransform(LegIK_Transforms[i].BoneIndex);
			FTransform current_leg_transform = LegIK_Transforms[i].Transform;

			LegIK_Transforms[i].Transform = UKismetMathLibrary::TLerp(original_leg_transform, current_leg_transform, toggle_alpha);
		...
		for (int i = 0; i < HandIK_Transforms.Num(); i++)
		{
			...
			if (Enable_Hand_Interpolation && test_counter > 100)
...
void FAnimNode_DragonAimSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	if (Context.AnimInstanceProxy->GetSkelMeshComponent())
	{
		...
		owner_skel_w_transform.SetRotation(Extra_Aiming_Rotation_Offset.Quaternion()*owner_skel_w_transform.GetRotation());
...
bool FAnimNode_DragonAimSolver::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
	...
	bool aim_input_validity = true;


	if(Use_Advanced_Input_Mode)
	{

		if (Bone_Structure_Precise.Bone_Input.Num() == 0 || Bone_Structure_Initialized.Num() == 0)
		{
			return false;
		}
		...
		for (int i = 0; i < Bone_Structure_Initialized.Num(); i++)
		{

			if (Bone_Structure_Initialized[i].IsValidToEvaluate(RequiredBones) == false)
				return false;

		}

		/*
		for (int i = 0; i < Bone_Structure_Precise.Bone_Input.Num(); i++)
		{

			FBoneReference Bone_Input = FBoneReference(Bone_Structure_Precise.Bone_Input[i]);
			Bone_Input.Initialize(RequiredBones);

			if(Bone_Input.IsValidToEvaluate(RequiredBones) == false)
			 aim_input_validity = false;
	...
	else
	{
		...
		if(( EndSplineBone.IsValidToEvaluate(RequiredBones) && StartSplineBone.IsValidToEvaluate(RequiredBones) && RequiredBones.BoneIsChildOf(EndSplineBone.BoneIndex, StartSplineBone.BoneIndex)) )
		{

		}
		else
		{
			aim_input_validity = (( EndSplineBone.IsValidToEvaluate(RequiredBones) && StartSplineBone.IsValidToEvaluate(RequiredBones) && RequiredBones.BoneIsChildOf(EndSplineBone.BoneIndex, StartSplineBone.BoneIndex)));
		}
	...
	{
		return (RequiredBones.BoneIsChildOf(FBoneReference(dragon_bone_data.Start_Spine).BoneIndex, FBoneReference(dragon_bone_data.Pelvis).BoneIndex) && aim_input_validity && RequiredBones.IsValid());
	}

	return (aim_input_validity && RequiredBones.IsValid() );
...
void FAnimNode_DragonAimSolver::InitializeBoneReferences(FBoneContainer& RequiredBones)
{
	...
	if(owning_skel)
	{
		...
		//dragon_input_data.Pelvis = owning_skel->GetBoneName(0);
	...
	Bone_Structure_Initialized.Empty();

	if(Use_Advanced_Input_Mode && Bone_Structure_Precise.Bone_Input.Num() > 0)
	{

		EndSplineBone = FBoneReference(Bone_Structure_Precise.Bone_Input[ Bone_Structure_Precise.Bone_Input.Num()-1 ]);

		StartSplineBone = FBoneReference(Bone_Structure_Precise.Bone_Input[0]);


		for (int b_i = 0; b_i < Bone_Structure_Precise.Bone_Input.Num(); b_i++)
		{
			FBoneReference Bone_Struct_Ref = FBoneReference(Bone_Structure_Precise.Bone_Input[b_i]);
			Bone_Struct_Ref.Initialize(*SavedBoneContainer);
			Bone_Structure_Initialized.Add(Bone_Struct_Ref);
		}
		...
		if (input_bone.ToString().TrimStartAndEnd().Equals(total_spine_bones[i].ToString().TrimStartAndEnd()))
		{
			if (spine_Feet_pair.Num() > i)
			{
				...
				//if (!automatic_leg_make)
				{
					...
					FBoneReference second_knee_ref = FBoneReference(dragon_input_data.FeetBones[index].Second_Knee_Bone_Name);
					second_knee_ref.Initialize(*SavedBoneContainer);
					dragon_bone_data.SecondKneeBones.Add(second_knee_ref); 
	...
	//	if (toggle_alpha > 0.01f)
	{
		...
		if (Aggregate_Hand_Body && Use_Separate_Targets)
		{
			for (int arm_index = 0; arm_index < Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides.Num(); arm_index++)
			{
				...
				if (((owning_skel->GetWorld()->WorldType != EWorldType::EditorPreview) || Work_Outside_PIE))
				{
		...
		if (enable_solver)
		{
			...
			if(Use_Advanced_Input_Mode)
			{
				...
				if(Bone_Structure_Precise.Bone_Input.Num() > 1)
				{
					FBoneReference Adv_Bone_Ref = FBoneReference(Bone_Structure_Precise.Bone_Input[ Bone_Structure_Precise.Bone_Input.Num()-1 ]);
					Adv_Bone_Ref.Initialize(*SavedBoneContainer);

					if(Adv_Bone_Ref.IsValidToEvaluate())
					 Head_Orig_Transform = MeshBases.GetComponentSpaceTransform(Adv_Bone_Ref.CachedCompactPoseIndex);
				}
		...
		//	LookAtLocation_Temp = LookAtLocation_Saved;
			...
			*/
			...
			////
			FVector EndChainLoc = owner_skel_w_transform.TransformPosition( MeshBases.GetComponentSpaceTransform(StartSplineBone.CachedCompactPoseIndex).GetLocation() );
			EndChainLoc.Z = Head_World.Z;

			FVector End_To_Target_Diff = LookAtLocation_Temp.GetLocation() - EndChainLoc;

			LookAtLocation_Temp.SetLocation(EndChainLoc + End_To_Target_Diff.GetSafeNormal()*FMath::Max(End_To_Target_Diff.Length(),Aim_Frontal_Offset));
			////
...
/*
			if(owning_skel->GetWorld() && owning_skel!=nullptr)
			{

				FHitResult outhit;
				TArray<AActor*> ignoreActors;
			//	UKismetSystemLibrary::SphereTraceSingle(owning_skel->GetWorld(), LookAtLocation_Temp.GetLocation(), LookAtLocation_Temp.GetLocation()+FVector(0,0,1), 2, Trace_Channel, false, ignoreActors, EDrawDebugTrace::ForOneFrame, outhit, true, FColor::White);
			}
*/		
			...
			if (Lock_Legs)
				for (int i = 0; i < spine_Feet_pair.Num(); i++)
				{
					...
					//int common_bone_index = owning_skel->GetBoneIndex(dragon_input_data.Pelvis);
					...
					if (is_valid_bone)
						for (int j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
						{
							...
							{
								int second_knee_bone_index = owning_skel->GetBoneIndex(dragon_input_data.FeetBones[i].Second_Knee_Bone_Name);
								bool is_second_knee_valid = second_knee_bone_index > -1;
							//	bool is_second_knee_valid = owning_skel->GetBoneIndex(dragon_bone_data.SecondKneeBones[spine_Feet_pair[i].order_index[j]].BoneName) > -1;
								...
								//ONLY USED IF THE SECOND KNEE BONE IS A VALID BONE NAME.
								if(is_second_knee_valid)
								{
								//	Thigh_Transform = MeshBases.GetComponentSpaceTransform(dragon_bone_data.SecondKneeBones[spine_Feet_pair[i].order_index[j]].CachedCompactPoseIndex);
									Thigh_Transform = MeshBases.GetComponentSpaceTransform(FCompactPoseBoneIndex(second_knee_bone_index));


									FTransform UpperThighTransform =  MeshBases.GetComponentSpaceTransform(spine_Feet_pair[i].Associated_Thighs[j].CachedCompactPoseIndex);
									//UpperThighTransform = UpperThighTransform* RootBone_Transform.Inverse();
									FVector Original_Upper_Second_Dir = (UpperThighTransform.GetLocation() - Thigh_Transform.GetLocation()).GetUnsafeNormal();
									
									
									FTransform Inv_Upper_Leg = UpperThighTransform * RootBone_Transform.Inverse();
									UpperThighTransform = Inv_Upper_Leg * HeadTransforms[common_bone_index].Transform;

									//FVector Upper_Feet_Dir = (UpperThighTransform.GetLocation() - MeshBases.GetComponentSpaceTransform(spine_Feet_pair[i].Associated_Feet[j].CachedCompactPoseIndex).GetLocation()).GetUnsafeNormal();
									//FVector Upper_Feet_Dir = (UpperThighTransform.GetLocation() - MeshBases.GetComponentSpaceTransform(dragon_bone_data.SecondKneeBones[spine_Feet_pair[i].order_index[j]].CachedCompactPoseIndex).GetLocation()).GetUnsafeNormal();

									FTransform Clamped_LowerThighProjectedTransform = Thigh_Transform * RootBone_Transform.Inverse() * HeadTransforms[common_bone_index].Transform;
									FTransform Feet_Transform = MeshBases.GetComponentSpaceTransform(spine_Feet_pair[i].Associated_Feet[j].CachedCompactPoseIndex);
									FVector Upper_Feet_Dir1 = (Clamped_LowerThighProjectedTransform.GetLocation() - Feet_Transform.GetLocation());
									float Upper_Feet_Max_Len = (Feet_Transform.GetLocation() - Thigh_Transform.GetLocation()).Size();
									//LowerThighProjectedTransform.SetLocation();
									Clamped_LowerThighProjectedTransform.SetLocation(Feet_Transform.GetLocation()+Upper_Feet_Dir1.GetUnsafeNormal()*Upper_Feet_Max_Len);
									

									FVector Upper_Feet_Dir = (UpperThighTransform.GetLocation() - Clamped_LowerThighProjectedTransform.GetLocation()).GetUnsafeNormal();

									//FIND ROTATION DIFFERENCE BETWEEN (ORIG THIGH-SECKNEE) and (CURRENT THIGH-SECKNEE) BONE)
									FQuat UpperThighRotOffset = FQuat::FindBetweenNormals(Original_Upper_Second_Dir,Upper_Feet_Dir);

									UpperThighTransform.SetRotation(UpperThighTransform.GetRotation()*UpperThighRotOffset);
									//dragon_bone_data.SecondKneeBones[spine_Feet_pair[i].order_index[j]] = FBoneReference(dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Second_Knee_Bone_Name);;
									//dragon_bone_data.SecondKneeBones[spine_Feet_pair[i].order_index[j]].Initialize(*SavedBoneContainer);
									
									LegIK_Transforms.Add(FBoneTransform(spine_Feet_pair[i].Associated_Thighs[j].CachedCompactPoseIndex,UpperThighTransform));


									Thigh_Transform = Thigh_Transform*(MeshBases.GetComponentSpaceTransform(spine_Feet_pair[i].Associated_Thighs[j].CachedCompactPoseIndex).Inverse() * UpperThighTransform);
								//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "is_second_knee_valid ");

								}
								else
								{
									Thigh_Transform = MeshBases.GetComponentSpaceTransform(spine_Feet_pair[i].Associated_Thighs[j].CachedCompactPoseIndex);
					...
					if (Main_Arm_Index > -1 && Main_Arm_Index < Hand_Array.Num())
					{
						...
						if (Actual_Shoulder_Array[Main_Arm_Index].IsValidToEvaluate() && Elbow_Array[Main_Arm_Index].IsValidToEvaluate() && Hand_Array[Main_Arm_Index].IsValidToEvaluate())
						{
							...
							if (Connector_Index.GetInt() > 0)
							{
								...
								if(Main_Arm_Index < Per_Limbs_Clamp_Array.Custom_Limb_Clamp_Array.Num())
								{

									Individual_Leg_Clamp = Per_Limbs_Clamp_Array.Custom_Limb_Clamp_Array[Main_Arm_Index];
								...
								if (!ignore_elbow_modification)
								{
									...
									if(Extra_Struct_Elbow_Pole_Array.elbow_offset_array.Num() > Main_Arm_Index && Extra_Struct_Elbow_Pole_Array.elbow_offset_array.Num() > 0)
									arm_ik_params.Extra_Elbow_Pole_Offset = Extra_Struct_Elbow_Pole_Array.elbow_offset_array[Main_Arm_Index];


									arm_ik_params.should_shoulder_twist_if_inward = Aiming_Hand_Limbs[Main_Arm_Index].should_shoulder_twist_if_inward;

									arm_ik_params.clavicle_only_bend_if_limit = Aiming_Hand_Limbs[Main_Arm_Index].clavicle_part_of_extension;
				...
				for (int limb_index = 0; limb_index < Aiming_Hand_Limbs.Num(); limb_index++)
				{
					...
					if ((Actual_Shoulder_Array[limb_index].IsValidToEvaluate() && Elbow_Array[limb_index].IsValidToEvaluate() && Hand_Array[limb_index].IsValidToEvaluate()) && (limb_index != Main_Arm_Index || Main_Arm_Index < 0 || reach_instead))
					{
						...
						if (Connector_Index.GetInt() > 0)
						{
							...
							if(limb_index < Per_Limbs_Clamp_Array.Custom_Limb_Clamp_Array.Num())
							{

								Individual_Leg_Clamp = Per_Limbs_Clamp_Array.Custom_Limb_Clamp_Array[limb_index];
							...
							//	}
								...
								if (Extra_Struct_Elbow_Pole_Array.elbow_offset_array.Num() > limb_index && Extra_Struct_Elbow_Pole_Array.elbow_offset_array.Num() > 0)
								arm_ik_params.Extra_Elbow_Pole_Offset = Extra_Struct_Elbow_Pole_Array.elbow_offset_array[limb_index];
								else
									arm_ik_params.Extra_Elbow_Pole_Offset = FVector(0,0,0);


								arm_ik_params.should_shoulder_twist_if_inward = Aiming_Hand_Limbs[limb_index].should_shoulder_twist_if_inward;
								arm_ik_params.clavicle_only_bend_if_limit = Aiming_Hand_Limbs[limb_index].clavicle_part_of_extension;
...
FVector FAnimNode_DragonAimSolver::AnimLocLerp(FVector start_pos, FVector end_pos, float delta_seconds)
{
	...
	if (!Enable_Interpolation || test_counter < 10)
	{
		return end_pos;
	}
