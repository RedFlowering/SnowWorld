...
void FAnimNode_DragonSpineSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	if (spine_Transform_pairs.Num() > 0)
	{
		...
		{
			if (spine_Feet_pair.Num() > i && (i == 0 || spine_Feet_pair[i].Associated_Feet.Num() > 0))
			{
				...
				if (is_snake || dragon_bone_data.FeetBones.Num() == 0 || spine_Feet_pair[i].Associated_Feet.Num())
				{
			...
			//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " Num()! "+FString::SanitizeFloat(spine_Feet_pair[i].Associated_Feet.Num()));


				//avoid traces for unnecessary spine points
			//	if(!feet_centre_point.ContainsNaN())
				{
...
void FAnimNode_DragonSpineSolver::InitializeBoneReferences(FBoneContainer& RequiredBones)
{
	...
	//if (spine_Feet_pair.Num() == 0)
	{
		...
		// Letting the LODs be always be updated
		...
		if (spine_Feet_pair.Num() > 1)
		{
			...
			//owning_skel->GetSkeletalMeshAsset()->GetLODInfo(0)->BonesToRemove;

			TArray<FBoneReference> removedbones = owning_skel->GetSkeletalMeshAsset()->GetLODInfo(owning_skel->GetPredictedLODLevel())->BonesToRemove;
			

			for (int bone_i = 0; bone_i < removedbones.Num(); bone_i++)
			{
				int RemovedBoneIndex = owning_skel->GetBoneIndex(removedbones[bone_i].BoneName);
				for (int combined_bone_i = 0; combined_bone_i < combined_indices.Num(); combined_bone_i++)
				{
					if (combined_indices[combined_bone_i].GetInt() == RemovedBoneIndex)
					{
						combined_indices.RemoveAt(combined_bone_i);
					}
				}
			}
			...
			if (spine_between_transforms.Num() == 0)
			{
				...
				*/
				...
				spine_between_transforms.SetNum(combined_indices.Num() - 2);
				spine_hit_edges.SetNum(combined_indices.Num() - 2);
				spine_between_offseted_transforms.SetNum(combined_indices.Num() - 2);

				spine_between_heights.SetNum(combined_indices.Num() - 2);

				snake_spine_positions.SetNum(combined_indices.Num());
			}

		//	GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, " Anti-Channel Detected " + FString::SanitizeFloat(combined_indices.Num()));
...
void FAnimNode_DragonSpineSolver::line_trace_func(USkeletalMeshComponent& skelmesh, FVector startpoint, FVector endpoint, FHitResult RV_Ragdoll_Hit, FName bone_text, FName trace_tag, FHitResult& Output, FLinearColor debug_color, bool debug_mode, bool ducking_mode)
{
	...
	if (owning_skel->GetOwner() && (trace_timer_count > current_trace_interval && trace_distance_legal) && !startpoint.ContainsNaN() && !endpoint.ContainsNaN())
	{
	...
	if (spine_Feet_pair.Num() > 0)
	{
		...
		else
		{
			...
			if (Hold_Snake_Root_Bone && owning_skel!= nullptr && Original_AnimatedBoneTransforms.Num() > 1 && test_counter > 20)
			{
				...
				if (Original_AnimatedBoneTransforms[0].Transform.IsRotationNormalized())
				{
