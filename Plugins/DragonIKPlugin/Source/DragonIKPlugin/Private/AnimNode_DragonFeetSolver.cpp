				...
				{
					{
						...
						if (frontal_finger_hits && sideway_feet_hits)
						{
							...
							if(!Use_Four_Point_Feets)
							{
								right_cross = UKismetMathLibrary::TransformDirection(owner_skel_w_transform,FVector::CrossProduct(character_forward_direction_vector_CS,character_direction_vector_CS));
							}
						...
						//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " -- : " + forward_impact.ToString()+" ____ "+right_cross.ToString());
						...
						*/
						////////////
						//normal_finger_impact = FVector(0,1,0);
...
FRotator FAnimNode_DragonFeetSolver::RotationFromImpactNormal(int spine_index,int feet_index,bool is_finger,FComponentSpacePoseContext& Output, FVector normal_impact_input, FTransform original_Bone_transform, float feet_limit)
{
	...
	//OLD METHOD FRotator normal_rot = FRotator(UKismetMathLibrary::DegAtan2(normal_impact.X, normal_impact.Z) * -1, 0, UKismetMathLibrary::DegAtan2(normal_impact.Y, normal_impact.Z) * 1);
	FRotator normal_rot = FRotator( FQuat::FindBetweenNormals(character_direction_vector_CS,normal_impact) );
	
	FVector unit_normal_impact = character_direction_vector_CS;
	//OLD METHOD FRotator unit_normal_rot = FRotator(UKismetMathLibrary::DegAtan2(unit_normal_impact.X, unit_normal_impact.Z) * -1, 0, UKismetMathLibrary::DegAtan2(unit_normal_impact.Y, unit_normal_impact.Z) * 1);
	FRotator unit_normal_rot = FRotator( FQuat::FindBetweenNormals(character_direction_vector_CS,unit_normal_impact) );
	...
	//if (!is_finger)
	{
		...
		TestTransform1.SetRotation(Overall_Feet_Rotation_Offset.Quaternion() * TestTransform1.GetRotation());
	...
	if (owning_skel)
	{
		...
		for (int i = 0; i < spine_Feet_pair.Num(); i++)
		{
			for (int j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
			{
				...
				FQuat Mesh_Rotation_Transform_Difference = FQuat::FindBetweenNormals(character_direction_vector_CS,FVector::UpVector);

				bonetraceTransform = bonetraceTransform*owner_skel_w_transform;
				FTransform Rotated_Owner_Skel_Transform = owner_skel_w_transform;
				Rotated_Owner_Skel_Transform.SetRotation(Rotated_Owner_Skel_Transform.GetRotation()*Mesh_Rotation_Transform_Difference);

				bonetraceTransform = bonetraceTransform*Rotated_Owner_Skel_Transform.Inverse();
			...
			//	if (Automatic_Foot_Height_Detection)
				...
				if (!dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Fixed_Foot_Height)
				{
					if (dragon_input_data.FeetBones.Num() > spine_Feet_pair[i].order_index[j])
					{
						...
						FeetRootHeights[i].Add((FMath::Abs(bonetraceTransform.GetLocation().Z) * owning_skel->GetComponentScale().Z) + ((dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Feet_Heights + Height_Extra) * owning_skel->GetComponentScale().Z));
...
void FAnimNode_DragonFeetSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	if (Context.AnimInstanceProxy->GetSkelMeshComponent())
	{
		...
		if (Context.AnimInstanceProxy->GetSkelMeshComponent()->GetOwner())
		{
		...
		for (int32 i = 0; i < spine_hit_pairs.Num(); i++)
		{
			//if (spine_Transform_pairs[i].Associated_Feet.Num() > 0 && spine_hit_pairs[i].RV_Feet_Hits.Num() > 0)
			{
				...
				if (i < spine_Feet_pair.Num() && i < spine_Transform_pairs.Num())
					...
					{
						FTransform eb_wtransform = spine_Transform_pairs[i].Associated_Temp_Feet[j];
						...
						feet_impactpoint_array[i][j] = eb_wtransform.GetLocation();
						spine_Transform_pairs[i].Associated_Temp_Feet[j] = eb_wtransform*owner_skel_w_transform;
	...
	//if (enable_solver)
	{
		...
		//bool FootStepCalledFrame = false;

		if(footstep_listening_counter < 200)
			footstep_listening_counter++;
							...
							//	if ((  spine_Feet_pair[i].order_index[j]) < sticky_feets_data.sticky_feet_array.Num() )
								{
									...
									if (sticky_feet_bool == true && spine_hit_pairs[i].RV_Feet_Hits[j].bBlockingHit)
									{
										...
										int twod_to_oned_index = spine_Feet_pair[i].order_index[j];

										if(sticky_feets_data.sticky_feet_alpha_array.Num() > twod_to_oned_index)
										{
											alpha_multiplier = sticky_feets_data.sticky_feet_alpha_array[twod_to_oned_index];
										}
										
										spine_Transform_pairs[i].Associated_Sticky_Alpha[j] = FMath::FInterpConstantTo(spine_Transform_pairs[i].Associated_Sticky_Alpha[j], 1*alpha_multiplier, delta_seconds_saved, sticky_feet_on_speed);
								...
								else
									{
									    ...
									    spine_Transform_pairs[i].Associated_Frozen_Feet[j].SetRotation(spine_Transform_pairs[i].Associated_Feet[j].GetRotation());
						...
						//	offseted_linetrace_location += (UKismetMathLibrary::TransformLocation(owner_skel_w_transform, dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Feet_Trace_Offset));
							...
							*/
							...
							if (use_footstep_listening )
							{
								...
								if (dragon_footsteps_component != nullptr)
								{
									...
									if ((z_difference < dragon_footsteps_component->foot_enter_height))
									{
										if (FeetStepBools[i][j] == false )
										{
											...
											if( footstep_listening_counter > 5)
											{
												...
												footstep_listening_counter = 0;
											}
											

											FeetStepBools[i][j] = true;

											//FootStepCalledFrame = true;
...
void FAnimNode_DragonFeetSolver::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (solver_time_reference > 0.15f && execute_in_animbp && enable_solver && (FVector(0, 0, 0) - Output.AnimInstanceProxy->GetActorTransform().GetScale3D()).Size() > 0 && FAnimWeight::IsRelevant(ActualAlpha) && IsValidToEvaluate(Output.AnimInstanceProxy->GetSkeleton(), Output.AnimInstanceProxy->GetRequiredBones()) && spine_Feet_pair.Num() > 0 && Output.ContainsNaN() == false)
	{
...
void FAnimNode_DragonFeetSolver::Leg_Full_Function(FName foot_name, int spine_index, int feet_index, FComponentSpacePoseContext& MeshBasesSaved, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if(solver_time_reference < 0.25f)
...
void FAnimNode_DragonFeetSolver::Leg_ik_Function(FBoneReference ik_footbone, int spine_index, int feet_index, TEnumAsByte<enum EBoneControlSpace> EffectorLocationSpace, TEnumAsByte<enum EBoneControlSpace> JointTargetLocationSpace, FComponentSpacePoseContext& MeshBasesSaved, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	FTransform Extra_Limb_Offset = FTransform::Identity;
	...
	// = (*SavedBoneContainer).GetParentBoneIndex(UpperLimbIndex);
	FCompactPoseBoneIndex SecondUpperLimbIndex = dragon_bone_data.SecondKneeBones[spine_Feet_pair[spine_index].order_index[feet_index]].CachedCompactPoseIndex;

	FTransform SecondUpperLimbCSTransform = FTransform::Identity;

	bool SecondUpperLimbValid = false;
	
	if(SecondUpperLimbIndex.IsValid())
	{
		SecondUpperLimbCSTransform = MeshBasesSaved.Pose.GetComponentSpaceTransform(SecondUpperLimbIndex);
		SecondUpperLimbValid = true;
	}
	...
	FVector RootPos = UpperLimbCSTransform.GetTranslation();
	FVector InitialJointPos = LowerLimbCSTransform.GetTranslation();
	FVector InitialEndPos = EndBoneCSTransform.GetTranslation();

	FTransform Original_EndBoneCSTransform = EndBoneCSTransform;
	...
	if (!first_time_setup && atleast_one_hit)
	{
		...
		//	if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit)
		{
...

			{
				...
				if (spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].bBlockingHit)
				{
					...
					//Temp_Impact_Ref = Temp_Impact_Ref-dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Trace_Offset;
					...
					FVector Impact_Original_Abs_Dir = (Temp_Impact_Ref - EndBoneCSTransform.GetLocation())*(Absolute_Up_Vector);
					...
					if (sticky_feet_mode)
					{
						if (sticky_floor_detection)
						{
							...
							//	Temp_Impact_Ref.Y = EndBoneCSTransform.GetLocation().Y;
								...
								Temp_Impact_Ref = EndBoneCSTransform.GetLocation()+Impact_Original_Abs_Dir;
							}
					...
					{
						//SET THE X and Y to original position to reduce jitters caused by the trace intervals (revisit needed..)
						//Temp_Impact_Ref.X = EndBoneCSTransform.GetLocation().X;
						//Temp_Impact_Ref.Y = EndBoneCSTransform.GetLocation().Y;
						...
						Temp_Impact_Ref += extra_foot_offset;
				...
				*/
				...
				FVector normal_impact_for_slope = UKismetMathLibrary::InverseTransformDirection(owner_skel_w_transform, spine_hit_pairs[spine_index].RV_Feet_Hits[feet_index].ImpactNormal);
				FRotator normal_rot_for_slope = FRotator(UKismetMathLibrary::DegAtan2(normal_impact_for_slope.X, normal_impact_for_slope.Z) * -1, 0, UKismetMathLibrary::DegAtan2(normal_impact_for_slope.Y, normal_impact_for_slope.Z) * 1);

			//	float pitch_data = ( (FRotator(normal_rot_for_slope).Pitch),(FRotator(normal_rot_for_slope).Roll) );
			//	float pitch_data = FMath::Max( (FRotator(normal_rot_for_slope).Pitch),(FRotator(normal_rot_for_slope).Roll) );
			// REVISIT THIS LATER..
				float pitch_data = FRotator(normal_rot_for_slope).Roll;

			//	GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " pitch_data : " + FString::SanitizeFloat(pitch_data));

				
			//	float pitch_data = FRotator(normal_rot_for_slope).Pitch;
					
				float dip_offset_multiplier = 0;
				float dip_blended_alpha1 = UKismetMathLibrary::NormalizeToRange( FMath::Clamp(pitch_data,0,10),0,10);
				float dip_blended_alpha2 = 1 - UKismetMathLibrary::NormalizeToRange(FMath::Clamp(pitch_data,-10,0),-10,0);
		
				if(pitch_data > 0)
				{
					dip_offset_multiplier = FMath::Lerp(0, dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Downslope_Offset_Multiplier,dip_blended_alpha1);
				}
				else
				{
					dip_offset_multiplier = FMath::Lerp(0, dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Upslope_Offset_Multiplier,dip_blended_alpha2);
				}
				float extra_slope_offset = FMath::Max(FMath::Abs(FRotator(normal_rot_for_slope).Roll), FMath::Abs(FRotator(normal_rot_for_slope).Pitch)) * dip_offset_multiplier * scale_mode;
				...
				Temp_Impact_Ref.Z += extra_slope_offset;

				

				if (Ignore_Location_Lerping || test_counter < 20 || solver_time_reference < 0.2f)
				{
				...
				else
				{
					...
					//	pitch_dataGEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " Delta_Loc_Speed : " + FString::SanitizeFloat(Delta_Loc_Speed));

					}
					else
					{

						//float dotangle = FMath::Acos(FVector::DotProduct(UKismetMathLibrary::TransformDirection(owner_skel_w_transform, Absolute_Up_Vector),Absolute_Up_Vector));
						//GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " dotangle : " + FString::SanitizeFloat(dotangle));

						//FVector Hit_Original_Abs_Dir;
						//FVector Hit_Original_Abs_Dir = (feet_impactpoint_array[spine_index][feet_index] - Temp_Impact_Ref)*(UKismetMathLibrary::TransformDirection(owner_skel_w_transform, Absolute_Up_Vector));
						...
						/*
						if(character_direction_vector_CS.Equals(FVector(0,0,1)))
						{
							Hit_Original_Abs_Dir = (feet_impactpoint_array[spine_index][feet_index] - Temp_Impact_Ref)*(character_direction_vector_CS);
						}
						else*/
						{
							...
							Hit_Original_Abs_Dir = (feet_impactpoint_array[spine_index][feet_index] - Temp_Impact_Ref)*(SimpleDirections[ClosestIndex]);
						}


						
					//	GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " ClosestIndex : " + FString::SanitizeFloat(ClosestIndex));
						

						
						

					//	FVector Z_Interpolated = FVector(Temp_Impact_Ref.X,Temp_Impact_Ref.Y,feet_impactpoint_array[spine_index][feet_index].Z);
						
						feet_impactpoint_array[spine_index][feet_index] = Temp_Impact_Ref + Hit_Original_Abs_Dir;
	...
	//	float pitch_data = ( (FRotator(Rotated_Difference).Pitch),(FRotator(Rotated_Difference).Roll) );
		float pitch_data = ( (FRotator(normal_rot_for_slope).Pitch),(FRotator(normal_rot_for_slope).Roll) );

		float dip_offset_multiplier = 0;

		float dip_blended_alpha1 = UKismetMathLibrary::NormalizeToRange( FMath::Clamp(pitch_data,0,20),0,20);
		float dip_blended_alpha2 = 1 - UKismetMathLibrary::NormalizeToRange(FMath::Clamp(pitch_data,-20,0),-20,0);
		
		//dip_offset_multiplier = FMath::Lerp(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Downslope_Offset_Multiplier,dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Downslope_Offset_Multiplier,pitch_data);
	...
	//	GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " Feet Root Pitch : " + normal_rot_for_slope.ToString()+" --- "+FString::SanitizeFloat(dip_offset_multiplier));
...
*/
		...
		//if(first_time_count < 10)
		//	feet_Alpha_array[spine_index][feet_index] = dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Feet_Alpha* alpha_multiplier;
	}
	...
	if(SecondUpperLimbIndex.IsValid() && SecondUpperLimbValid && test_counter > 5)
	{

		//float LowerLimbLength = (InitialEndPos - InitialJointPos).Size();
		//float UpperLimbLength = (InitialJointPos - RootPos).Size();

		float Limb_Max_Length = (InitialEndPos - InitialJointPos).Size() + (InitialJointPos - RootPos).Size();
		float Limb_Current_Length = (EffectorTransform.GetTranslation() - RootPos).Size();
		Limb_Current_Length = FMath::Clamp<float>(Limb_Current_Length,0,Limb_Max_Length);
		
		float ratio_selection = UKismetMathLibrary::NormalizeToRange(Limb_Current_Length,Limb_Max_Length*0.01f,Limb_Max_Length*1.0f);
		//ratio_selection = 0.5f;
		
		FVector Impact_Feet_Difference = ((UKismetMathLibrary::InverseTransformLocation(owner_skel_w_transform,feet_impactpoint_array[spine_index][feet_index])) -Original_EndBoneCSTransform.GetLocation())*ratio_selection;

		float Root_Feet_Length = (InitialEndPos - RootPos).Size();//*FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Extension)

		float Limb_Extension_Remainder = (Limb_Current_Length - Root_Feet_Length*FMath::Abs(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Max_Feet_Extension));
		Limb_Extension_Remainder = -1*FMath::Abs<float>(Limb_Extension_Remainder);
		Impact_Feet_Difference.Z = FMath::Clamp<float>(Impact_Feet_Difference.Z,Limb_Extension_Remainder,100000000.0f);
		
		//////
		//FOR SECOND KNEE LOGIC
		Extra_Limb_Offset.SetLocation(Impact_Feet_Difference);

		//UpperLimbCSTransform.AddToTranslation(FVector(0,0,-400));
		
		EndBoneCSTransform.AddToTranslation(Extra_Limb_Offset.GetTranslation());
		UpperLimbCSTransform.AddToTranslation(Extra_Limb_Offset.GetTranslation());
		LowerLimbCSTransform.AddToTranslation(Extra_Limb_Offset.GetTranslation());

		Original_EndBoneCSTransform.AddToTranslation(Extra_Limb_Offset.GetTranslation());
		RootPos += (Extra_Limb_Offset.GetTranslation());
		InitialJointPos += (Extra_Limb_Offset.GetTranslation());
		InitialEndPos += (Extra_Limb_Offset.GetTranslation());
		//////
	...
	FQuat Forward_Rotation_Difference = FQuat::FindBetweenNormals(poles_forward_direction_vector_CS,character_forward_direction_vector_CS);
...
//	FRP_Knee_Transform.SetLocation(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].Knee_Direction_Offset);
	...
	/*
	if(SecondUpperLimbValid)
	{
		UpperLimbLength = (InitialJointPos - SecondUpperLimbCSTransform.GetLocation()).Size();
	}*/
	
	float MaxLimbLength = LowerLimbLength + UpperLimbLength;

    
	//GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, " OutEndPos : " + FString::SanitizeFloat(MaxLimbLength));
...
//		FVector world_impact_point_CS = owner_skel_w_transform.InverseTransformPosition(feet_impactpoint_array[spine_index][feet_index]);
	...
	if(SecondUpperLimbValid)
	{
		...
		FBoneReference SecondUpperBoneRef = FBoneReference(owning_skel->GetBoneName(SecondUpperLimbIndex.GetInt()));
		SecondUpperBoneRef.Initialize(*SavedBoneContainer);

		FTransform LowerLimbCSTransform_Temp_Orig = LowerLimbCSTransformX;
		FTransform UpperLimbCSTransform_Temp_Orig = UpperLimbCSTransformX;
		FTransform SecondUpperLimbCSTransform_Temp_Orig = SecondUpperLimbCSTransform;

	//	LowerLimbCSTransform_Temp_Orig.AddToTranslation(Extra_Limb_Offset.GetTranslation());
	//	UpperLimbCSTransform_Temp_Orig.AddToTranslation(-Extra_Limb_Offset.GetTranslation());
		
	//	AnimationCore::SolveTwoBoneIK(SecondUpperLimbCSTransform,UpperLimbCSTransform,LowerLimbCSTransform,UpperLimbCSTransform.GetLocation(),LowerLimbCSTransform.GetLocation(),true,1,5);
		
		FQuat ExtraAlteredUpperOffset = FQuat::FindBetweenNormals((UpperLimbCSTransform_Temp_Orig.GetLocation()-SecondUpperLimbCSTransform.GetLocation()).GetUnsafeNormal(),(UpperLimbCSTransform.GetLocation() - SecondUpperLimbCSTransform.GetLocation()).GetUnsafeNormal());

		SecondUpperLimbCSTransform.SetRotation(ExtraAlteredUpperOffset*SecondUpperLimbCSTransform.GetRotation());


		FTransform Lerped_SecondKneeCSTransform = UKismetMathLibrary::TLerp(SecondUpperLimbCSTransform_Temp_Orig, SecondUpperLimbCSTransform, Alpha_Temp);
		
		OutBoneTransforms.Add(FBoneTransform(SecondUpperLimbIndex, Lerped_SecondKneeCSTransform));


		
		
		//UpperLimbCSTransform.SetRotation(UpperLimbCSTransformX.GetRotation());
///////////
		FTransform Orig_Trans_LArm_Rel_To_UArm = UpperLimbCSTransform_Temp_Orig*SecondUpperLimbCSTransform_Temp_Orig.Inverse();
		Orig_Trans_LArm_Rel_To_UArm = Orig_Trans_LArm_Rel_To_UArm* SecondUpperLimbCSTransform;

		FVector Second_IK_Offset = (Orig_Trans_LArm_Rel_To_UArm.GetLocation() - UpperLimbCSTransform.GetLocation());
		UpperLimbCSTransform.AddToTranslation(Second_IK_Offset );
		LowerLimbCSTransform.AddToTranslation(Second_IK_Offset);

		FTransform EndPointCached = EndBoneCSTransform;
		EndBoneCSTransform.AddToTranslation(Second_IK_Offset);
		
		AnimationCore::SolveTwoBoneIK( UpperLimbCSTransform,LowerLimbCSTransform, EndBoneCSTransform,JointTargetTransform.GetLocation(),EndPointCached.GetLocation(),false,1,1);

		
	}


	//spine_Transform_pairs[spine_index].Associated_Frozen_Feet[feet_index].SetRotation(feet_mod_transform_array[spine_index][feet_index].GetRotation());

	if(sticky_feet_mode && Lock_Feet_Rotation)
	{

		if(feet_frozen_rotator_array[spine_index][feet_index].Equals(FRotator::ZeroRotator))
		{
			feet_frozen_rotator_array[spine_index][feet_index] = FRotator(owner_skel_w_transform.GetRotation()*(feet_mod_transform_array[spine_index][feet_index]).GetRotation());
		}
		...
		if (sticky_floor_detection)
		{
			...
			if (spine_AnimatedTransform_pairs[spine_index].Sticky_Feet_Auto[feet_index])
			{
				...
				FQuat Frozen_Rotation_Value = (owner_skel_w_transform.GetRotation().Inverse() *(feet_frozen_rotator_array[spine_index][feet_index]).Quaternion());
				EndBoneCSTransform.SetRotation(FQuat::Slerp(EndBoneCSTransform.GetRotation(),Frozen_Rotation_Value,spine_Transform_pairs[spine_index].Associated_Sticky_Alpha[feet_index]));
		...
		//EndBoneCSTransform.SetRotation(Frozen_Rotation_Value);
		...
		if(test_counter < 100)
		{
			...
			if (owning_skel->GetOwner() != nullptr)
			{
				...
				FTransform actor_rotation_world = owning_skel->GetOwner()->GetActorTransform();

				FQuat Actor_Skel_Rotator_Diff = owner_skel_w_transform.GetRotation();

				//feet_frozen_rotator_array[spine_index][feet_index] = ( feet_mod_transform_array[spine_index][feet_index]* ( actor_rotation_world*owner_skel_w_transform.Inverse() )).Rotator();

				feet_frozen_rotator_array[spine_index][feet_index] = FRotator(Actor_Skel_Rotator_Diff*(feet_mod_transform_array[spine_index][feet_index]).GetRotation());

			}
		}
		*/

		//EndBoneCSTransform.SetRotation(owner_skel_w_transform.GetRotation().Inverse() *(feet_frozen_rotator_array[spine_index][feet_index]).Quaternion());
...
//	EndBoneCSTransform.SetRotation(owner_skel_w_transform.GetRotation().Inverse() *(feet_frozen_rotator_array[spine_index][feet_index]).Quaternion());
	...
	//dragon_input_data.FeetBones[spine_Feet_pair[i].order_index[j]].Finger_Array[k_finger]
	for (int feet_child_index = 0; feet_child_index < dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].FeetFollowBones.Num(); feet_child_index++)
	{

		FBoneReference feet_follow_bone = FBoneReference(dragon_input_data.FeetBones[spine_Feet_pair[spine_index].order_index[feet_index]].FeetFollowBones[feet_child_index]);
		feet_follow_bone.Initialize(*SavedBoneContainer);

		if(feet_follow_bone.IsValidToEvaluate(*SavedBoneContainer))
		{
			FTransform Feet_Original_Offset_Transform = EndBoneCSTransformX.Inverse()*Lerped_EndBoneCSTransform;
		
			FTransform FeetFollow_Transform = MeshBasesSaved.Pose.GetComponentSpaceTransform(feet_follow_bone.CachedCompactPoseIndex);
			FeetFollow_Transform = FeetFollow_Transform*Feet_Original_Offset_Transform;

			OutBoneTransforms.Add(FBoneTransform(feet_follow_bone.CachedCompactPoseIndex, FeetFollow_Transform));
		}
	...
	if (Affect_Toes_Always)
	{
		...
		for (int fingerx = 0; fingerx < spine_hit_pairs[spine_index].RV_Associated_Fingers_Hits[feet_index].Num(); fingerx++)
		{
			...
			FTransform Inv_FootValue = EndBoneCSTransformX.Inverse() * Lerped_EndBoneCSTransform;

			FTransform Feet_Finger_Offset = ComponentBoneTransform_Finger*EndBoneCSTransformX.Inverse();
			...
			feet_fingers_transform_array[spine_index][feet_index][fingerx].SetLocation((Feet_Finger_Offset*Lerped_EndBoneCSTransform).GetLocation());
...
void FAnimNode_DragonFeetSolver::InitializeBoneReferences(FBoneContainer& RequiredBones)
{
	...
	//UE_ASSUME(owning_skel != nullptr);


	//if (first_time_count < 3)
	//if(is_initialized == false)
	{
		//GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, " Test Counter : ");
		solve_should_fail = false;

		if (owning_skel)
		{

			PelvisBoneName = owning_skel->GetBoneName(0);

			SpineBoneName = owning_skel->GetBoneName(1);

		}

		if (!SpineBoneName.IsNone())
		{
			dragon_bone_data.Start_Spine = FBoneReference(SpineBoneName);
			dragon_bone_data.Start_Spine.Initialize(RequiredBones);

			dragon_bone_data.Pelvis = FBoneReference(PelvisBoneName);
			...
			*/
			...
			if (solve_should_fail == false)
			{
				...
				if (test_counter < 10)
				{
					...
					feet_frozen_rotator_array.Empty();
					feet_fingers_transform_array.Empty();
					feet_knee_offset_array.Empty();
				...
				Total_spine_bones = BoneArrayMachine(0, SpineBoneName, PelvisBoneName, false);
				...
				for (int32 i = 0; i < dragon_input_data.FeetBones.Num(); i++)
				{
					...
					bool is_second_knee_valid = owning_skel->GetBoneIndex(dragon_input_data.FeetBones[i].Second_Knee_Bone_Name) > -1;


					if (is_second_knee_valid)
						BoneArrayMachine_Feet(i, dragon_input_data.FeetBones[i].Feet_Bone_Name, dragon_input_data.FeetBones[i].Knee_Bone_Name, dragon_input_data.FeetBones[i].Second_Knee_Bone_Name, PelvisBoneName, true);
					else
						BoneArrayMachine_Feet(i, dragon_input_data.FeetBones[i].Feet_Bone_Name, dragon_input_data.FeetBones[i].Knee_Bone_Name, dragon_input_data.FeetBones[i].Thigh_Bone_Name, PelvisBoneName, true);

				}


				if (PelvisBoneName == SpineBoneName)
				{
					solve_should_fail = true;
				}
				...
				for (int32 i = 0; i < spine_Feet_pair.Num(); i++)
				{
					if (spine_Feet_pair[i].Associated_Feet.Num() == 0)
						spine_Feet_pair.RemoveAt(i, 1, EAllowShrinking::Yes);
				...
				if (test_counter < 10)
				{
					...
					feet_frozen_rotator_array.AddDefaulted(spine_Feet_pair.Num());
				...
				for (int32 i = 0; i < spine_Feet_pair.Num(); i++)
				{
					...
					*/
					...
					if (test_counter < 10)
					{
						...
						feet_frozen_rotator_array[i].AddDefaulted(spine_Feet_pair[i].Associated_Feet.Num());
						...
						for (int feet_x = 0; feet_x < spine_Transform_pairs[i].Associated_Sticky_Alpha.Num(); feet_x++)
						{
							spine_Transform_pairs[i].Associated_Sticky_Alpha[feet_x] = 1;
						}
				...
				if (SpineBoneName == PelvisBoneName)
					solve_should_fail = true;
				...
				*/
				...
				dragon_bone_data.SecondKneeBones.Empty();
				...
				for (int i = 0; i < dragon_input_data.FeetBones.Num(); i++)
				{
					...
					//	if (automatic_leg_make == false)
					{
						...
						if (is_second_knee_valid)
						{
							...
							dragon_bone_data.SecondKneeBones.Add(FBoneReference(dragon_input_data.FeetBones[i].Thigh_Bone_Name));
							dragon_bone_data.SecondKneeBones[i].Initialize(RequiredBones);

							dragon_bone_data.ThighBones.Add(FBoneReference(dragon_input_data.FeetBones[i].Second_Knee_Bone_Name));
							dragon_bone_data.ThighBones[i].Initialize(RequiredBones);
						}
						else
						{
							dragon_bone_data.SecondKneeBones.Add(FBoneReference("None"));
							dragon_bone_data.SecondKneeBones[i].Initialize(RequiredBones);
	...
	if (owning_skel->GetOwner())
	{
		if (trace_timer_count > current_trace_interval && trace_distance_legal)
		{
			...
			if (Use_Anti_Channel)
			{
				...
				if(use_trace_manager_component)
				{
					FName Trace_Identifier = FName("F"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
					UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), startpoint - character_direction_vector * line_trace_upper_height * scale_mode, startpoint,0,true, Anti_Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, Fdebug_color,Trace_Identifier,bone_text);
				}
				else
				{
				...
				//	if (RV_Anti_Checker.GetComponent())
				{
					...
					if (response_channel == ECollisionResponse::ECR_Block)
					{
						...
						if (trace_type == EIKTrace_Type_Plugin::ENUM_LineTrace_Type || Selected_Trace_Radius < 0.2f || is_line)
						{
							...
							{
								FName Trace_Identifier = FName("F"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
								UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), New_StartPoint, endpoint,0,true, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, Fdebug_color,Trace_Identifier,bone_text);
							}
							else
							{
							...
							{//(startpoint-endpoint).Size()
								...
								{
									FName Trace_Identifier = FName("F"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
									UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), New_StartPoint + character_direction_vector * Scaled_Trace_Radius, endpoint + character_direction_vector * Scaled_Trace_Radius,Scaled_Trace_Radius,false, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, Fdebug_color,Trace_Identifier,bone_text);
								}
								else
								{
			...
			else
			{
				...
				if ((trace_type == EIKTrace_Type_Plugin::ENUM_LineTrace_Type || Selected_Trace_Radius < 0.2f) || is_line)
				{
					...
					{
						FName Trace_Identifier = FName("F"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
						UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), startpoint, endpoint,0,true, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, Fdebug_color,Trace_Identifier,bone_text);
					}
					else
					{
						UKismetSystemLibrary::LineTraceSingle(owning_skel->GetOwner(), startpoint, endpoint, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, Fdebug_color);
					}
					
					//if(Dragon_Trace_Manager != nullptr)
					// GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " ImpactNormal " + RV_Ragdoll_Hit.ImpactNormal.ToString()+" and ");
					...
					{//(startpoint-endpoint).Size()
						...
						{
							FName Trace_Identifier = FName("F"+FString::FromInt(solver_identifier)+FString::FromInt(Identifier_Counter));
							UDragonIK_Library::TraceSelectorFire(Dragon_Trace_Manager,owning_skel->GetOwner(), startpoint, endpoint,Scaled_Trace_Radius,false, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, Fdebug_color,Trace_Identifier,bone_text);
						}
						else
						{
							UKismetSystemLibrary::SphereTraceSingle(owning_skel->GetOwner(), startpoint + character_direction_vector * Scaled_Trace_Radius, endpoint + character_direction_vector * Scaled_Trace_Radius, Scaled_Trace_Radius, Trace_Channel, line_trace_hit_complex, ignoreActors, EDrawDebugTrace::None, RV_Ragdoll_Hit, true, Fdebug_color);
						}
					}
				/*
					else
