...
void FAnimNode_DragonWeaponArmSolver::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if(turn_on_solver && Hand_Array.Num() > 0)
	{
...
/* OLD CODE FOR AUTO REFERENCING THE WEAPON HANDLER COMPONENT!
...
*/			
		...
		if(Internal_Switched_Interpolation_bool)
		{
			if (weapon_handler_component != nullptr)
			{
						...
						if(weapon_handler_component->arm_reset_queue_index.Num() > 0 && alpha_index < weapon_handler_component->arm_reset_queue_index.Num())
							if (weapon_handler_component->arm_reset_queue_index[alpha_index] == true)
							{
								...
								if (grab_alpha[alpha_index] == 0 && Calculated_Hand_Transforms.Num() > 0 && weapon_handler_component->active_weapon_component)
								{
									...
									if (USkeletalMeshComponent* Casted_SkeletalMeshComponent = Cast<USkeletalMeshComponent>(weapon_handler_component->active_weapon_component))
									{
										int weapon_bone_index = Casted_SkeletalMeshComponent->GetBoneIndex(weapon_handler_component->weapon_bone);
										weapon_transform = Casted_SkeletalMeshComponent->GetBoneTransform(weapon_bone_index);
									}
				...
				bool grab_detection_status = false;

				

				for(int grab_i = 0; grab_i<Aiming_Hand_Limbs.Num() ;grab_i++)
				{

					if(owning_skel->GetBoneIndex(weapon_handler_component->weapon_grabbed_bone) == Aiming_Hand_Limbs[grab_i].Hand_Bone_Name.BoneIndex )
					{

						grab_detection_status = true;
					}

					
				}
				FName wep_grabbed_bone_selected = weapon_handler_component->weapon_grabbed_bone;

				if(grab_detection_status == false)
					wep_grabbed_bone_selected = Aiming_Hand_Limbs[primary_hand_index].Hand_Bone_Name.BoneName;
				...
				if (owning_skel->GetBoneIndex(wep_grabbed_bone_selected) > -1 && weapon_handler_component->active_weapon_component)
				{
					...
					//dominant_bone_reference = Hand_Bone_Switch;
...
///////////////
					...
					FBoneReference ik_bone_reference = FBoneReference(weapon_handler_component->weapon_grabbed_bone);
					ik_bone_reference.Initialize(*SavedBoneContainer);
					FTransform IK_current_bone_transform = (Output.Pose.GetComponentSpaceTransform(ik_bone_reference.CachedCompactPoseIndex));
					//IK_current_bone_transform.SetRotation(owning_skel->GetRelativeRotation().Quaternion());

					FBoneReference real_hand_bone_reference = FBoneReference(wep_grabbed_bone_selected);
					real_hand_bone_reference.Initialize(*SavedBoneContainer);
					FTransform real_hand_bone_transform = (Output.Pose.GetComponentSpaceTransform(real_hand_bone_reference.CachedCompactPoseIndex));
					
					IK_current_bone_transform.SetRotation(  real_hand_bone_transform.GetRotation() );
					
					//IK_current_bone_transform.SetRotation(owning_skel->GetRelativeRotation().Quaternion()*FRotator(0,-90,0).Quaternion());
					
					//IK_current_bone_transform.SetRotation(IK_current_bone_transform.GetRotation()*FRotator(0,-90,0).Quaternion());
					IK_current_bone_transform = IK_current_bone_transform*owning_skel->GetComponentTransform();
					...
					///////////
					///////
					FTransform Modified_Initial_IK_Hand_Transform = Initial_IK_Hand_Transform;
					Modified_Initial_IK_Hand_Transform.SetRotation(real_hand_bone_transform.GetRotation());
                    //Frozen initial transform of the grabbed bone, converted to world space
					FTransform initial_handtransform_ws = Modified_Initial_IK_Hand_Transform*owning_skel->GetComponentTransform();

					IK_Hand_Real_Offset = ( initial_handtransform_ws)*(IK_current_bone_transform.Inverse());
...
//////////////					
					...
					//check if hand bone switch is a proper hand
					FBoneReference True_Hand_Bone;

					for (int limb_index = 0; limb_index < Aiming_Hand_Limbs.Num(); limb_index++)
					{
						if (Aiming_Hand_Limbs[limb_index].Hand_Bone_Name.BoneName.IsEqual(wep_grabbed_bone_selected))
 						 True_Hand_Bone = Aiming_Hand_Limbs[limb_index].Hand_Bone_Name;

					}
					
					if(True_Hand_Bone.BoneIndex == -1)
						True_Hand_Bone = Aiming_Hand_Limbs[primary_hand_index].Hand_Bone_Name;
					...
					for (int limb_index = 0; limb_index < Aiming_Hand_Limbs.Num(); limb_index++)
					{
						if (Aiming_Hand_Limbs[limb_index].Hand_Bone_Name.BoneName.IsEqual(True_Hand_Bone.BoneName))
						{
					...
					//GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Yellow, " weapon_handler_component...."+weapon_handler_component->dominant_hand_transform.ToString());
					if (Actual_Shoulder_Array.Num() > shoulder_index && Actual_Shoulder_Array.Num() > 0 && shoulder_index > -1)
					{
						...
						//weapon_handler_component->dominant_hand_name= wep_grabbed_bone_selected;
						weapon_handler_component->dominant_hand_name= Hand_Array[shoulder_index].BoneName;
					...
					//weapon_handler_component->dominant_hand_transform = FTransform(FVector(100,100,100));
...
bool FAnimNode_DragonWeaponArmSolver::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
	...
	if(weapon_handler_component == nullptr && test_counter > 0)
	{
		return false;
	}
		...
		//if (Use_Separate_Targets)
		{
			...
			{			
				...
				for (int arm_index = 0; arm_index < Arm_TargetLocation_Overrides.Arm_TargetLocation_Overrides.Num(); arm_index++)
				{
					...
					if ((Calculated_Hand_Transforms.Num() > arm_index && Hand_Array.Num() > arm_index) && weapon_handler_component != nullptr )
					{

						if (weapon_handler_component->active_weapon_component != nullptr )
						{

							FTransform Weapon_World_Transform = weapon_handler_component->active_weapon_component->GetComponentTransform();

							if (USkeletalMeshComponent* Casted_SkeletalMeshComponent = Cast<USkeletalMeshComponent>(weapon_handler_component->active_weapon_component))
							{
								int weapon_bone_index = Casted_SkeletalMeshComponent->GetBoneIndex(weapon_handler_component->weapon_bone);
								Weapon_World_Transform = Casted_SkeletalMeshComponent->GetBoneTransform(weapon_bone_index);
							}
							
							
							Calculated_Hand_Transforms[arm_index] = Weapon_World_Transform;
							...
							FTransform weapon_transform = Weapon_World_Transform;
							//weapon_transform.SetRotation(FQuat::Identity);
							weapon_transform.SetScale3D(FVector::OneVector);
							...
							FCompactPoseBoneIndex ik_bone = ik_bone_reference.CachedCompactPoseIndex;
							FTransform ik_bone_transform = (Output.Pose.GetComponentSpaceTransform(ik_bone));
							...
							//if (weapon_handler_component->perform_grab_weapon)
							{
								...
								if (Calibration_Status_Array[arm_index] == false)
								{
									...
									if(Calibration_Frame_Tick_Count > 3)
									{
										Calibration_Status_Array[arm_index] = true;
										Calibration_Frame_Tick_Count = 0;
									}
									else
									{
										Calibration_Frame_Tick_Count++;
									}

								//	IK_Hand_Distance =   (ik_bone_transform*owner_skel_w_transform).GetLocation() - (MeshBases.GetComponentSpaceTransform(Hand_Array[primary_hand_index].CachedCompactPoseIndex)*owner_skel_w_transform).GetLocation();

									Frozen_Hand_Transform = MeshBases.GetComponentSpaceTransform(Hand_Array[primary_hand_index].CachedCompactPoseIndex);
									{
										//ik_bone_transform.SetRotation(owning_skel->GetRelativeRotation().Quaternion());
										Frozen_Weapon_Transform = weapon_transform;

										Relative_Frozen_Weapon_Transform = weapon_transform*owner_skel_w_transform.Inverse();
										
										Initial_IK_Hand_Transform = ik_bone_transform;
										//Initial_IK_Hand_Transform.SetRotation(owning_skel->GetRelativeRotation().Quaternion());
										
										Frozen_RelMesh_Transform = owning_skel->GetRelativeTransform();

										if(owning_skel->GetOwner() != nullptr)
 										 Frozen_Pawn_Transform = owning_skel->GetOwner()->GetActorTransform();
									}
							...
							FTransform ik_bone_transform_ws = ik_bone_transform*owner_skel_w_transform;

							FTransform frozen_ik_bone_transform_ws = Initial_IK_Hand_Transform*owner_skel_w_transform;
							...
							FTransform Original_Hand_Transform = Calculated_Hand_Transforms[arm_index];

							FTransform Weapon_Transform_Virtual =   (Relative_Frozen_Weapon_Transform*owner_skel_w_transform)*(frozen_ik_bone_transform_ws).Inverse()*ik_bone_transform_ws;
							//ALGO FOR PARENT-CHILD : CHILD*(INITIAL_PARENT_TRANSFORM.INV()*CURRENT_PARENT_TRANSFORM)
							...
							//if(arm_index == primary_hand_index)
							{
                            							
								Calculated_Hand_Transforms[arm_index] = IK_Hand_Real_Offset*Calculated_Hand_Transforms[arm_index];
								//Calculated_Hand_Transforms[arm_index] = Calculated_Hand_Transforms[arm_index]*IK_Hand_Real_Offset;



								//POSITION THE WEAPON TRANSFORM INTO THE LOCATION OF THE INPUT-HAND BONE
							//	weapon_transform.SetLocation((ik_bone_transform*owner_skel_w_transform).GetLocation());

								//GET THE PRACTICAL-HAND TRANSFORM RELATIVE TO THE WEAPON TRANSFORM. THIS IS AFTER THE WEAPON TRANSFORM IS SET TO THE INPUT-HAND LOCATION
								FTransform HandRelativeTransform_To_Weapon = Calculated_Hand_Transforms[arm_index]*(weapon_transform.Inverse());

								//STRO : SKELETAL COMPONENT IN WORLD SPACE, THEN TWEAK IT'S ROTATION ALONE.
								FTransform skel_transform_relative_offseted = owner_skel_w_transform;

					
								if(owning_skel->GetOwner() != nullptr)
								{

									skel_transform_relative_offseted = FTransform::Identity;
								}
							

								//FWTI IS FROZEN WEAPON TRANSFORM MULTIPLIED WITH STRO.
								//FTransform Frozen_Weapon_Transform_Inverted = Frozen_Weapon_Transform*(skel_transform_relative_offseted);
								FTransform Frozen_Weapon_Transform_Inverted = Frozen_Weapon_Transform*(skel_transform_relative_offseted);
							
								//	FTransform RotatedRelativeTransform = RelativeTransform*FTransform( weapon_transform.GetRotation().Inverse()*Frozen_Weapon_Transform_Inverted.GetRotation());
								//IKWO IS FROZEN WEAPON TRANSFORM RELATIVE TO FROZEN IK-HAND TRANSFORM. AN OFFSET USEFUL LATER ON.
								FTransform IK_Weapon_Offset = Frozen_Weapon_Transform*(Initial_IK_Hand_Transform*Frozen_RelMesh_Transform.GetRotation()*Frozen_Pawn_Transform.GetRotation()).Inverse();
								//	FTransform RotatedRelativeTransform = HandRelativeTransform_To_Weapon*FTransform( (IK_Weapon_Offset*ik_bone_transform_ws).GetRotation().Inverse()*Frozen_Weapon_Transform_Inverted.GetRotation());

								//......
								FTransform RotatedRelativeTransform = HandRelativeTransform_To_Weapon*FTransform( (IK_Weapon_Offset*(ik_bone_transform*Frozen_RelMesh_Transform.GetRotation()*Frozen_Pawn_Transform.GetRotation())).GetRotation().Inverse()*Frozen_Weapon_Transform_Inverted.GetRotation());
							
								Calculated_Hand_Transforms[arm_index] = RotatedRelativeTransform*weapon_transform;

								FTransform Pure_Calculated_Hand_Transform = RotatedRelativeTransform*Weapon_Transform_Virtual;

								Calculated_Hand_Transforms[arm_index] = Original_Hand_Transform*(Pure_Calculated_Hand_Transform.Inverse()*Calculated_Hand_Transforms[arm_index]);
