...
void UDragonIKPhysicsComponent::FirstTimeInitialization(USkeletalMeshComponent* skeleton_input)
{
	...
	if (skeleton_input != nullptr && owning_skeleton == nullptr)
	{
		/*
...
{
	Multicast_DragonIKSimulatePhysics(skmesh,BoneName,IncludeSelf);
}
...
{
	skmesh->SetAllBodiesBelowSimulatePhysics(BoneName,true,IncludeSelf);
}
...
{
	Multicast_CacheSimulationStates();
}
	...
	if (owning_skeleton != nullptr && component_grabbing_state && use_auto_blending_logic && is_released == false) //
	{
		...
		if ( ragdoll_mode == false && selected_bone_name!="None")
		{
			...
			for (int i = 0; i < Bodies_Physics_Hit_Bools.Num(); i++)
			{
				...
				if(Influence_Stopping_Bones.Contains(selected_bone_name))
				{
					parent_index = hit_bone_index;
				}
...
void UDragonIKPhysicsComponent::Server_OnHitCallback_Implementation(const 
	FName Optional_Override_Hit_Bone_Name)
{
	Multicast_OnHitCallback(Optional_Override_Hit_Bone_Name);
}

bool UDragonIKPhysicsComponent::Server_OnHitCallback_Validate(const 
	FName Optional_Override_Hit_Bone_Name)
{
	return true;
}

void UDragonIKPhysicsComponent::Multicast_OnHitCallback_Implementation(const 
	FName Optional_Override_Hit_Bone_Name)
{
	FVector Hit_Impulse_Var = FVector::ZeroVector;
	FVector Hit_Dir_Var = FVector::ZeroVector;
	FName Hit_Bone_Name_Var = "";
	FHitResult Hit_Temp;
	OnHitCallback(Hit_Temp,Optional_Override_Hit_Bone_Name,Hit_Impulse_Var,Hit_Dir_Var,Hit_Bone_Name_Var);
}

bool UDragonIKPhysicsComponent::Multicast_OnHitCallback_Validate(const 
	FName Optional_Override_Hit_Bone_Name)
{
	return true;
}
...
void UDragonIKPhysicsComponent::Server_Modify_Reset_Bone_Info_Implementation(const TArray<FName>& bone_list)
{
	...
	if(bone_list.Num() > 0)
	{
		Multicast_Modify_Reset_Bone_Info(bone_list);
	}
}
...
{
	//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, "bone_listNUM "+ FString::SanitizeFloat(bone_list.Num()));
	Modify_Reset_Bone_Info(bone_list);
}
...
void UDragonIKPhysicsComponent::CreateAllHandles()
{
	...
	//UE_ASSUME(owning_skeleton != nullptr);
	//UE_ASSUME(handler_holder != nullptr);
...
/////////////////// TELEPORTATION LOGIC
			...
			if((get_up_timer > 1 && !is_released))
			{
				if(!Body_Delta_Transform.Equals(FTransform::Identity) && (Body_Delta_Transform.GetLocation() - owning_skeleton->GetComponentTransform().GetLocation()).Length() > Teleport_Distance)
				{
				...
				//	bool Blending_Finished = false;

				//	if(Bodies_Physics_Blend.Num() > 0)
				//	 Blending_Finished = Bodies_Physics_Blend[0] == 1;
				...
				if( teleport_freeze_count < 10 && teleport_freeze_count > 0)
				{
					...
					owning_skeleton->SetAllPhysicsLinearVelocity(FVector::ZeroVector,false);
					owning_skeleton->SetAllPhysicsAngularVelocityInDegrees(FVector::ZeroVector,false);
					//owning_skeleton->SetWorldTransform(Saved_Relative_Hand_Transform*owning_skeleton->GetSocketTransform(weapon_grabbed_bone,ERelativeTransformSpace::RTS_World));

					teleport_freeze_count++;


					if(teleport_freeze_count == 5)
					{
						owning_skeleton->ForEachBodyBelow(Custom_Root_Bone, true, false, [this](FBodyInstance* BI)
								{
									int bone_index = BI->InstanceBoneIndex;
					
									BI->SetBodyTransform(Raw_Animation_Transforms[bone_index],ETeleportType::TeleportPhysics);
					
								});
					}
				
					//GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, "physanim = ");
			...
			Body_Delta_Transform = owning_skeleton->GetComponentTransform();
...
{
	Multicast_ReGrabAllHandles(exit_ragdoll);
}
...
void UDragonIKPhysicsComponent::Server_SetInterpolation_Implementation(int inter_value)
{
	Multicast_SetInterpolation(inter_value);
}

bool UDragonIKPhysicsComponent::Server_SetInterpolation_Validate(int inter_value)
{
	if(inter_value <= 0)
	{
		return false;
...
void UDragonIKPhysicsComponent::Multicast_SetInterpolation_Implementation(int inter_value)
{
	SetInterpolation(inter_value);
}

bool UDragonIKPhysicsComponent::Multicast_SetInterpolation_Validate(int inter_value)
{
	if(inter_value <= 0)
	{
		return false;
...
void UDragonIKPhysicsComponent::UpdateSpecificPhysanimData(FName BoneName,int linear_strength_var, int angular_strength_var, int linear_damp_var, int angular_damp_var)
{
	...
	if (handler_holder != nullptr && owning_skeleton != nullptr)
	{
		for (int handle_i = 0; handle_i < handler_holder->physics_handle_array.Num(); handle_i++)
		{
			...
			if (handler_holder->physics_handle_array[handle_i] != nullptr && BoneName.IsEqual(bone_name))
			{
...
void UDragonIKPhysicsComponent::Server_UpdatePhysanimData_Implementation(int LinearStrength, int AngularStrength, int LinearDamp, int AngularDamp)
{
	Multicast_UpdatePhysanimData(LinearStrength,AngularStrength,LinearDamp,AngularDamp);
}

bool UDragonIKPhysicsComponent::Server_UpdatePhysanimData_Validate(int LinearStrength, int AngularStrength,
	int LinearDamp, int AngularDamp)
{
	return true;
}

void UDragonIKPhysicsComponent::Multicast_UpdatePhysanimData_Implementation(int LinearStrength, int AngularStrength,
	int LinearDamp, int AngularDamp)
{
	UpdatePhysanimData(LinearStrength,AngularStrength,LinearDamp,AngularDamp);
}

bool UDragonIKPhysicsComponent::Multicast_UpdatePhysanimData_Validate(int LinearStrength, int AngularStrength,
	int LinearDamp, int AngularDamp)
{
	return true;
}
...
{
	Multicast_ReleaseAllHandles(go_ragdoll);
}
...
void UDragonIKPhysicsComponent::Physanim_Safe_Exit_Ragdoll()
{
	...
	if (handler_holder != nullptr && owning_skeleton != nullptr)
	{
		...
		owning_skeleton->ForEachBodyBelow(root_bone, true, false, [this,clamp_index](FBodyInstance* BI)
		{
			int bone_index = BI->InstanceBoneIndex;
...
{
	Multicast_PerformActiveRagdoll(pelvis_mass_override);
}
...
{
	PerformActiveRagdoll(pelvis_mass_override);
}
...
bool UDragonIKPhysicsComponent::Server_Control_Bone_Handle_Validate(FName bone_name, FTransform override_transform,
	bool enable)
{
	return true;
}

void UDragonIKPhysicsComponent::Server_Control_Bone_Handle_Implementation(FName bone_name,
                                                                          FTransform override_transform, bool enable)
{
	//this->SetIsReplicated(true);

	Multicast_Control_Bone_Handle(bone_name,override_transform,enable);
}

void UDragonIKPhysicsComponent::Multicast_Control_Bone_Handle_Implementation(FName bone_name,
	FTransform override_transform, bool enable)
{
	Control_Bone_Handle(bone_name,override_transform,enable);
}

bool UDragonIKPhysicsComponent::Multicast_Control_Bone_Handle_Validate(FName bone_name, FTransform override_transform,
	bool enable)
{
	return true;
}
