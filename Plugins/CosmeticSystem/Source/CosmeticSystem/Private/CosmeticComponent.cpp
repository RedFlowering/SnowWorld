	...
	: Super(ObjectInitializer)
{
	...
	CachedMeshCollisionProfileName = UCollisionProfile::NoCollision_ProfileName;
}
...
void UCosmeticComponent::BeginPlay()
{
	...
	if (Character)
	{
		CachedMeshCollisionProfileName = Character->GetMesh()->GetCollisionProfileName();
...
void UCosmeticComponent::ApplyRetargetMeshPreset(FCosmeticItemID RetargetMeshPresetID)
 {	
	 ...
	 if (GetRetargetMeshData(CosmeticData.RetargetMeshPresetID, RetargetMeshData))
	 {
		 ...
		 OnChangedRetargetMeshPreset.Broadcast(RetargetMeshPresetID);
	 }
 }
	...
	{	
		...
		if (!ChildActor)
		{
			...
			{				
				...
				OnCreatedCosmeticActor.Broadcast(CosmeticActorInstance);

				if (Character->GetMovementComponent()->UpdatedPrimitive)
				{
					Character->GetMovementComponent()->UpdatedPrimitive->IgnoreActorWhenMoving(CosmeticActorInstance, true);
				}
			}
		}
		...
		if (CosmeticActorInstance)
		{
			...
			{			
				...
				Character->GetMesh()->SetCollisionProfileName(UCollisionProfile::NoCollision_ProfileName);				
			}
			else
			{
				MeshComp->SetPhysicsAsset(nullptr);			
				MeshComp->SetCollisionProfileName(UCollisionProfile::NoCollision_ProfileName);
				Character->GetMesh()->SetCollisionProfileName(CachedMeshCollisionProfileName);
			}

			for (auto& Elem : RetargetMeshData.Materials)
			{
				MeshComp->SetMaterialByName(Elem.Key, Elem.Value.LoadSynchronous());
			}
