...
USkeletalMeshComponent* UCosmeticBFL::GetVisualMesh(const ACharacter* Target)
{
	if (ACosmeticActor* CosmeticActor = GetCosmeticActor(Target))
	{
		return CosmeticActor->GetVisualMesh();
	}
	else
	{
		if (Target)
