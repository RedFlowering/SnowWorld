...
void UHarmoniaCharacterMovementComponent::PhysWalking(const float DeltaTime, int32 IterationsCount)
{
	...
	while ((remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) && CharacterOwner && (CharacterOwner->GetController() || bRunPhysicsWithNoController || HasAnimRootMotion() || CurrentRootMotion.HasOverrideVelocity() || (CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)))
	{
		...
		// check(EnsureAlwaysEnabledConsoleVariable != nullptr)
			...
			// Apply acceleration
			const bool bSkipForLedgeMove = bTriedLedgeMove && (EnsureAlwaysEnabledConsoleVariable && EnsureAlwaysEnabledConsoleVariable->GetBool());
...
void UHarmoniaCharacterMovementComponent::PhysCustom(const float DeltaTime, int32 IterationsCount)
{
	...
	case EHarmoniaCustomMovementMode::MOVE_None:
		Super::PhysCustom(DeltaTime, IterationsCount);
		break;
	case EHarmoniaCustomMovementMode::MOVE_GrapplingHook:
		PhysGrpplingHook(DeltaTime, IterationsCount);
		break;
	case EHarmoniaCustomMovementMode::MOVE_Climbing:
...
void UHarmoniaCharacterMovementComponent::PhysGrpplingHook(float DeltaTime, int32 IterationCount)
{
...
void UHarmoniaCharacterMovementComponent::PhysClimbing(float DeltaTime, int32 IterationCount)
{
	if (DeltaTime < MIN_TICK_TIME)
	{
		return;
...
FVector UHarmoniaCharacterMovementComponent::GetGrapplingHookMovmentVector()
{
	return GrapplingHookVector;
}

void UHarmoniaCharacterMovementComponent::SetGrapplingHookMovementVector(FVector GrapplingVector)
{
	GrapplingHookVector = GrapplingVector;
}

FVector UHarmoniaCharacterMovementComponent::GetClimbingMovementVector()
{
	return ClimbingVector;
}

void UHarmoniaCharacterMovementComponent::SetClimbingMovementVector(FVector ClimbVector)
{
