...
void UHarmoniaFishingWidget::ShowMinigameResult(bool bSuccess, bool bPerfect)
{
	if (MinigameResultText)
	{
		...
		if (bPerfect)
		{
			MinigameResultText->SetText(NSLOCTEXT("HarmoniaFishing", "PerfectCatch", "?ÑÎ≤Ω???öÏãú!"));
			MinigameResultText->SetColorAndOpacity(FLinearColor(1.0f, 0.84f, 0.0f)); // Gold
		}
		else if (bSuccess)
		{
			MinigameResultText->SetText(NSLOCTEXT("HarmoniaFishing", "GoodCatch", "?±Í≥µ!"));
			MinigameResultText->SetColorAndOpacity(FLinearColor::Green);
		}
		else
		{
			MinigameResultText->SetText(NSLOCTEXT("HarmoniaFishing", "FailedCatch", "?ìÏ≥§??.."));
...
void UHarmoniaFishingWidget::ShowCaughtFish(const FCaughtFish& FishInfo, const FFishData& FishData)
{
	...
	if (CaughtFishQualityText)
	{
		CaughtFishQualityText->SetText(FText::Format(
			NSLOCTEXT("HarmoniaFishing", "QualityFormat", "?àÏßà: {0}"),
...
void UHarmoniaFishingWidget::UpdateFishingSpotInfo(const UFishingSpotData* SpotData)
{
	...
	if (FishingSpotTypeText)
	{
		...
		case EFishingSpotType::River:
			TypeText = NSLOCTEXT("HarmoniaFishing", "SpotRiver", "Í∞?);
			break;
		case EFishingSpotType::Lake:
			TypeText = NSLOCTEXT("HarmoniaFishing", "SpotLake", "?∏Ïàò");
			break;
		case EFishingSpotType::Ocean:
			TypeText = NSLOCTEXT("HarmoniaFishing", "SpotOcean", "Î∞îÎã§");
			break;
		case EFishingSpotType::Pond:
			TypeText = NSLOCTEXT("HarmoniaFishing", "SpotPond", "?∞Î™ª");
			break;
		case EFishingSpotType::Special:
			TypeText = NSLOCTEXT("HarmoniaFishing", "SpotSpecial", "?πÎ≥Ñ ?öÏãú??);
			break;
		}
...
FText UHarmoniaFishingWidget::GetMinigameInstruction(EFishingMinigameType Type) const
{
	...
	{
	case EFishingMinigameType::TimingBased:
		return NSLOCTEXT("HarmoniaFishing", "InstructionTiming", "?Ä?¥Î∞ç??ÎßûÏ∂∞ Î≤ÑÌäº???ÑÎ•¥?∏Ïöî!");
	case EFishingMinigameType::BarBalance:
		return NSLOCTEXT("HarmoniaFishing", "InstructionBalance", "Í∑†Ìòï???†Ï??òÏÑ∏??");
	case EFishingMinigameType::QuickTimeEvent:
		return NSLOCTEXT("HarmoniaFishing", "InstructionQTE", "?úÏãú?òÎäî Î≤ÑÌäº??Îπ†Î•¥Í≤??ÑÎ•¥?∏Ïöî!");
	case EFishingMinigameType::ReelTension:
		return NSLOCTEXT("HarmoniaFishing", "InstructionTension", "?•Î†•???ÅÏ†à???†Ï??òÏÑ∏??");
	default:
...
FText UHarmoniaFishingWidget::GetRarityText(EFishRarity Rarity) const
{
	...
	case EFishRarity::Common:
		return NSLOCTEXT("HarmoniaFishing", "RarityCommon", "?ºÎ∞ò");
	case EFishRarity::Uncommon:
		return NSLOCTEXT("HarmoniaFishing", "RarityUncommon", "Í≥†Í∏â");
	case EFishRarity::Rare:
		return NSLOCTEXT("HarmoniaFishing", "RarityRare", "?¨Í?");
	case EFishRarity::Epic:
		return NSLOCTEXT("HarmoniaFishing", "RarityEpic", "?ÅÏõÖ");
	case EFishRarity::Legendary:
		return NSLOCTEXT("HarmoniaFishing", "RarityLegendary", "?ÑÏÑ§");
	default:
		return FText::GetEmpty();
	}
}
