...
{
	Ascending		UMETA(DisplayName = "Ascending"),		// ?§Î¶ÑÏ∞®Ïàú (??ùÑ?òÎ°ù Ï¢ãÏùå, ?? ?Ä?ÑÏñ¥??
	Descending		UMETA(DisplayName = "Descending"),		// ?¥Î¶ºÏ∞®Ïàú (?íÏùÑ?òÎ°ù Ï¢ãÏùå, ?? ?êÏàò)
	MAX				UMETA(Hidden)
};
...
{
	Numeric			UMETA(DisplayName = "Numeric"),			// ?´Ïûê (?êÏàò)
	Time			UMETA(DisplayName = "Time"),			// ?úÍ∞Ñ (Ï¥??®ÏúÑ)
	MAX				UMETA(Hidden)
};
