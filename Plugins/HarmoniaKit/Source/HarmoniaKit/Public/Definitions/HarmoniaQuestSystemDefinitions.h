...
{
	Locked			UMETA(DisplayName = "Locked"),			// ?†Í? (Ï°∞Í±¥ ÎØ∏Ï∂©Ï°?
	Available		UMETA(DisplayName = "Available"),		// ?òÎùΩ Í∞Ä??
	InProgress		UMETA(DisplayName = "In Progress"),		// ÏßÑÌñâ Ï§?
	ReadyToComplete	UMETA(DisplayName = "Ready to Complete"),// ?ÑÎ£å Í∞Ä??(Î™©Ìëú ?¨ÏÑ±)
	Completed		UMETA(DisplayName = "Completed"),		// ?ÑÎ£å??
	Failed			UMETA(DisplayName = "Failed"),			// ?§Ìå®
	MAX				UMETA(Hidden)
};
...
{
	Main			UMETA(DisplayName = "Main Quest"),		// Î©îÏù∏ ?òÏä§??
	Side			UMETA(DisplayName = "Side Quest"),		// ?¨Ïù¥???òÏä§??
	Daily			UMETA(DisplayName = "Daily Quest"),		// ?ºÏùº ?òÏä§??
	Weekly			UMETA(DisplayName = "Weekly Quest"),	// Ï£ºÍ∞Ñ ?òÏä§??
	Repeatable		UMETA(DisplayName = "Repeatable"),		// Î∞òÎ≥µ Í∞Ä??
	Story			UMETA(DisplayName = "Story Quest"),		// ?§ÌÜ†Î¶??òÏä§??
	Tutorial		UMETA(DisplayName = "Tutorial"),		// ?úÌÜ†Î¶¨Ïñº
	Achievement		UMETA(DisplayName = "Achievement"),		// ?ÖÏ†Å
	MAX				UMETA(Hidden)
};
...
{
	Kill			UMETA(DisplayName = "Kill Enemy"),		// ??Ï≤òÏπò
	Collect			UMETA(DisplayName = "Collect Item"),	// ?ÑÏù¥???òÏßë
	Talk			UMETA(DisplayName = "Talk to NPC"),		// NPC ?Ä??
	Reach			UMETA(DisplayName = "Reach Location"),	// ?ÑÏπò ?ÑÎã¨
	Craft			UMETA(DisplayName = "Craft Item"),		// ?ÑÏù¥???úÏûë
	Build			UMETA(DisplayName = "Build Structure"),	// Íµ¨Ï°∞Î¨?Í±¥ÏÑ§
	Equip			UMETA(DisplayName = "Equip Item"),		// ?ÑÏù¥???•Ï∞©
	Use				UMETA(DisplayName = "Use Item"),		// ?ÑÏù¥???¨Ïö©
	Gather			UMETA(DisplayName = "Gather Resource"),	// ?êÏõê Ï±ÑÏßë
	Deliver			UMETA(DisplayName = "Deliver Item"),	// ?ÑÏù¥???ÑÎã¨
	Escort			UMETA(DisplayName = "Escort NPC"),		// NPC ?∏ÏúÑ
	Defend			UMETA(DisplayName = "Defend Location"),	// ?ÑÏπò Î∞©Ïñ¥
	Discover		UMETA(DisplayName = "Discover Area"),	// ÏßÄ???êÌóò
	Custom			UMETA(DisplayName = "Custom"),			// Ïª§Ïä§?Ä (?úÍ∑∏ Í∏∞Î∞ò)
	MAX				UMETA(Hidden)
};
...
{
	None				UMETA(DisplayName = "None"),				// Ï°∞Í±¥ ?ÜÏùå
	QuestCompleted		UMETA(DisplayName = "Quest Completed"),		// ?†Ìñâ ?òÏä§???ÑÎ£å
	QuestNotCompleted	UMETA(DisplayName = "Quest Not Completed"),	// ?πÏ†ï ?òÏä§??ÎØ∏ÏôÑÎ£?
	Level				UMETA(DisplayName = "Level Requirement"),	// ?àÎ≤® ?îÍµ¨?¨Ìï≠
	HasItem				UMETA(DisplayName = "Has Item"),			// ?ÑÏù¥???åÏ?
	HasTag				UMETA(DisplayName = "Has Tag"),				// Í≤åÏûÑ?åÎ†à???úÍ∑∏ ?åÏ?
	RecipeKnown			UMETA(DisplayName = "Recipe Known"),		// ?àÏãú???µÎìù
	Custom				UMETA(DisplayName = "Custom"),				// Ïª§Ïä§?Ä Ï°∞Í±¥
	MAX					UMETA(Hidden)
};
...
enum class EQuestRewardType : uint8
{
	...
	Experience		UMETA(DisplayName = "Experience"),		// Í≤ΩÌóòÏπ?
	Gold			UMETA(DisplayName = "Gold"),			// Í≥®Îìú
	Item			UMETA(DisplayName = "Item"),			// ?ÑÏù¥??
	Recipe			UMETA(DisplayName = "Recipe"),			// ?àÏãú??
	Tag				UMETA(DisplayName = "Gameplay Tag"),	// Í≤åÏûÑ?åÎ†à???úÍ∑∏
	UnlockQuest		UMETA(DisplayName = "Unlock Quest"),	// ?òÏä§???¥Í∏à
	Custom			UMETA(DisplayName = "Custom"),			// Ïª§Ïä§?Ä Î≥¥ÏÉÅ
	MAX				UMETA(Hidden)
};
...
{
	Objective		UMETA(DisplayName = "Objective"),		// Ï£ºÏöî Î™©Ìëú
	Optional		UMETA(DisplayName = "Optional"),		// ?†ÌÉù Î™©Ìëú
	Discover		UMETA(DisplayName = "Discover"),		// ?êÌóò ÏßÄ??
	Deliver			UMETA(DisplayName = "Deliver"),			// ?ÑÎã¨ ÏßÄ??
	Talk			UMETA(DisplayName = "Talk"),			// ?Ä??NPC
	Gather			UMETA(DisplayName = "Gather"),			// Ï±ÑÏßë ÏßÄ??
...
{
	None			UMETA(DisplayName = "None"),			// ?ÜÏùå
	TimeLimit		UMETA(DisplayName = "Time Limit"),		// ?úÍ∞Ñ ?úÌïú
	NPCDied			UMETA(DisplayName = "NPC Died"),		// NPC ?¨Îßù
	ItemLost		UMETA(DisplayName = "Item Lost"),		// ?ÑÏù¥???êÏã§
	LocationLeft	UMETA(DisplayName = "Location Left"),	// ÏßÄ???¥ÌÉà
	PlayerDied		UMETA(DisplayName = "Player Died"),		// ?åÎ†à?¥Ïñ¥ ?¨Îßù
	Custom			UMETA(DisplayName = "Custom"),			// Ïª§Ïä§?Ä Ï°∞Í±¥
	MAX				UMETA(Hidden)
};
...
{
	OnStart			UMETA(DisplayName = "On Start"),		// ?òÏä§???úÏûë ??
	OnComplete		UMETA(DisplayName = "On Complete"),		// ?òÏä§???ÑÎ£å ??
	OnFail			UMETA(DisplayName = "On Fail"),			// ?òÏä§???§Ìå® ??
	OnAbandon		UMETA(DisplayName = "On Abandon"),		// ?òÏä§???¨Í∏∞ ??
	OnPhaseChange	UMETA(DisplayName = "On Phase Change"),	// ?®Í≥Ñ Î≥ÄÍ≤???
	OnObjectiveComplete UMETA(DisplayName = "On Objective Complete"), // Î™©Ìëú ?ÑÎ£å ??
	MAX				UMETA(Hidden)
};
...
{
	QuestAdded		UMETA(DisplayName = "Quest Added"),			// ?òÏä§??Ï∂îÍ?
	QuestStarted	UMETA(DisplayName = "Quest Started"),		// ?òÏä§???úÏûë
	QuestCompleted	UMETA(DisplayName = "Quest Completed"),		// ?òÏä§???ÑÎ£å
	QuestFailed		UMETA(DisplayName = "Quest Failed"),		// ?òÏä§???§Ìå®
	ObjectiveComplete UMETA(DisplayName = "Objective Complete"),// Î™©Ìëú ?ÑÎ£å
	PhaseComplete	UMETA(DisplayName = "Phase Complete"),		// ?®Í≥Ñ ?ÑÎ£å
	HintShown		UMETA(DisplayName = "Hint Shown"),			// ?åÌä∏ ?úÏãú
	MAX				UMETA(Hidden)
};
...
{
	None			UMETA(DisplayName = "None"),			// ?ÜÏùå
	SpawnActor		UMETA(DisplayName = "Spawn Actor"),		// ?°ÌÑ∞ ?§Ìè∞
	PlayCutscene	UMETA(DisplayName = "Play Cutscene"),	// Ïª∑Ïî¨ ?¨ÏÉù
	GrantReward		UMETA(DisplayName = "Grant Reward"),	// Î≥¥ÏÉÅ ÏßÄÍ∏?
	ModifyWorld		UMETA(DisplayName = "Modify World"),	// ?îÎìú Î≥ÄÍ≤?
	StartQuest		UMETA(DisplayName = "Start Quest"),		// ?òÏä§???úÏûë
	Custom			UMETA(DisplayName = "Custom"),			// Ïª§Ïä§?Ä
	MAX				UMETA(Hidden)
};
 ...
 */
USTRUCT(BlueprintType)
struct FHarmoniaQuestData : public FTableRowBase
{
	GENERATED_BODY()
...
struct FActiveQuestProgress
{
	...
	bool HasTimeLimit(const FHarmoniaQuestData& QuestData) const
	{
		return QuestData.TimeLimit > 0.0f;
	}

	// Get remaining time
	float GetRemainingTime(const FHarmoniaQuestData& QuestData) const
	{
	...
	// Check if time is up
	bool IsTimeUp(const FHarmoniaQuestData& QuestData) const
	{
