...
// ========================================
// Editor Visualization & Utility Functions
// ========================================

#if WITH_EDITOR

void UHarmoniaWorldGeneratorSubsystem::DrawBiomeDebugVisualization(
    UWorld* World,
    const TArray<FBiomeData>& BiomeData,
    const FWorldGeneratorConfig& Config,
    float Duration)
...
void UHarmoniaWorldGeneratorSubsystem::DrawRoadDebugVisualization(
    UWorld* World,
...
void UHarmoniaWorldGeneratorSubsystem::DrawRiverDebugVisualization(
    UWorld* World,
...
void UHarmoniaWorldGeneratorSubsystem::DrawStructureDebugVisualization(
    UWorld* World,
...
void UHarmoniaWorldGeneratorSubsystem::DrawCaveDebugVisualization(
    UWorld* World,
...
void UHarmoniaWorldGeneratorSubsystem::DrawResourceDebugVisualization(
    UWorld* World,
    ...
    float Duration)
{
    ...
    for (const FOreVeinData& Vein : OreVeins)
    {
        ...
        // Draw vein sphere
        DrawDebugSphere(World, Vein.CenterLocation, Vein.Radius, 12, VeinColor, false, Duration, 0, 5.0f);
        ...
        DrawDebugString(World, Vein.CenterLocation + FVector(0, 0, 100), ResourceName, nullptr, VeinColor, Duration);
    }
...
void UHarmoniaWorldGeneratorSubsystem::DrawPOIDebugVisualization(
    UWorld* World,
    ...
    float Duration)
{
    ...
    {
        // Assuming POI type is stored in scale or other property
        const FColor POIColor = FColor::Purple;
        ...
        // Draw label
        DrawDebugString(World, POI.Location + FVector(0, 0, 700), TEXT("POI"), nullptr, POIColor, Duration);
    }

    UE_LOG(LogTemp, Log, TEXT("Drew POI debug visualization (%d POIs)"), POIs.Num());
}

void UHarmoniaWorldGeneratorSubsystem::DrawHeightmapDebugVisualization(
    UWorld* World,
    const TArray<int32>& HeightData,
    const FWorldGeneratorConfig& Config,
    float Duration)
...
void UHarmoniaWorldGeneratorSubsystem::ClearDebugVisualizations(UWorld* World)
{
    if (!World)
    {
        return;
...
void UHarmoniaWorldGeneratorSubsystem::EditorQuickPreview(
    UWorld* World,
    const FWorldGeneratorConfig& Config,
    bool bShowBiomes,
    bool bShowRoads,
    ...
    bool bShowResources)
{
    ...
    GenerateHeightmapOnly(Config, HeightData);

    // Generate and visualize biomes
    if (bShowBiomes)
    {
        TArray<FBiomeData> BiomeData;
        GenerateBiomeMap(Config, HeightData, BiomeData);
    ...
    if (bShowRoads)
    {
        ...
        GenerateRoads(Config, HeightData, RoadSegments);
    ...
    if (bShowRivers)
    {
        ...
        GenerateRivers(Config, HeightData, BiomeData, RiverSegments);
    ...
    if (bShowStructures)
    {
        ...
        GenerateStructureGroups(Config, HeightData, DummyActorMap, StructureObjects);
    ...
    if (bShowResources)
    {
        ...
        GenerateResourceDistribution(Config, HeightData, BiomeData, OreVeins, ResourceNodes);
...
bool UHarmoniaWorldGeneratorSubsystem::ExportHeightmapToPNG(
    const TArray<int32>& HeightData,
    const FWorldGeneratorConfig& Config,
...
bool UHarmoniaWorldGeneratorSubsystem::ExportBiomeMapToPNG(
    const TArray<FBiomeData>& BiomeData,
    const FWorldGeneratorConfig& Config,
    const FString& FilePath)
{
...
FColor UHarmoniaWorldGeneratorSubsystem::GetBiomeDebugColor(EBiomeType BiomeType)
{
    switch (BiomeType)
    {
...
FColor UHarmoniaWorldGeneratorSubsystem::GetResourceDebugColor(EResourceType ResourceType)
{
    switch (ResourceType)
    {
...
FColor UHarmoniaWorldGeneratorSubsystem::GetPOIDebugColor(EPOIType POIType)
{
    switch (POIType)
    {
