...
#include "SensorRangedDistanceAndAngleTest.generated.h"
...
class SENSESYSTEM_API USensorRangedDistanceAndAngleTest : public USensorLocationTestBase
{
	GENERATED_BODY()

public:
	USensorRangedDistanceAndAngleTest(const FObjectInitializer& ObjectInitializer);
...
FORCEINLINE float USensorRangedDistanceAndAngleTest::ModifyDistanceScore(const float Value) const
{
	return DistanceCurve.EditorCurveData.Eval(FMath::Clamp(Value, 0.f, 1.f));
}
FORCEINLINE float USensorRangedDistanceAndAngleTest::ModifyAngleScore(const float Value) const
{
