...
#include "SensorSightRangedDistance.generated.h"


/**
 * Sensor Sight Ranged Distance
 */
UCLASS(Blueprintable, BlueprintType, EditInlineNew, HideCategories = (SensorTests))
class SENSESYSTEM_API USensorSightRangedDistance : public UActiveSensor
{
	GENERATED_BODY()

public:
	USensorSightRangedDistance(const FObjectInitializer& ObjectInitializer);
	virtual ~USensorSightRangedDistance() override;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "SensorSightRangedDistance")
	float MinScore = 0.f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "SensorSightRangedDistance", meta = (ClampMin = "0.0", UIMin = "0.0"))
