...
#include "Sensors/Tests/SensorRangedDistanceAndAngleTest.h"
...
USensorRangedDistanceAndAngleTest::USensorRangedDistanceAndAngleTest(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
#if WITH_EDITOR
void USensorRangedDistanceAndAngleTest::PostEditChangeProperty(struct FPropertyChangedEvent& e)
{
...
void USensorRangedDistanceAndAngleTest::SetDistanceAndAngleParam(
...
bool USensorRangedDistanceAndAngleTest::PreTest()
{
	Super::PreTest();
...
ESenseTestResult USensorRangedDistanceAndAngleTest::RunTestForLocation(const FSensedStimulus& SensedStimulus, const FVector& TestLocation, float& ScoreResult) const
{
	...
	if (AABB_Box.IsInsideOrOn(TestLocation))
	{
		...
		if ((MinDistanceSquared > DistSquared) || (MaxDistanceLostSquared < DistSquared))
		{
			ScoreResult = 0.0f;
			OutResult = ESenseTestResult::Lost;
			return OutResult;
		}
...
void USensorRangedDistanceAndAngleTest::InitializeCacheTest()
{
...
#if WITH_EDITORONLY_DATA
void USensorRangedDistanceAndAngleTest::DrawTest(const FSceneView* View, FPrimitiveDrawInterface* PDI) const
{
...
void USensorRangedDistanceAndAngleTest::DrawDebug(const float Duration) const
{
