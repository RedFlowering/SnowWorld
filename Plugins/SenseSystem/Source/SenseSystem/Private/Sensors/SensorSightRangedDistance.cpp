...
#include "Sensors/SensorSightRangedDistance.h"
#include "Sensors/Tests/SensorTraceTest.h"
#include "Sensors/Tests/SensorRangedDistanceAndAngleTest.h"

USensorSightRangedDistance::USensorSightRangedDistance(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	SensorTests[0] = CreateDefaultSubobject<USensorRangedDistanceAndAngleTest>(TEXT("RangedDistanceAndAngleTest"));
	SensorTests[1] = CreateDefaultSubobject<USensorTraceTest>(TEXT("TraceTest"));
	SetValuesToSensorTests();
}

USensorSightRangedDistance::~USensorSightRangedDistance()
{}

#if WITH_EDITOR
void USensorSightRangedDistance::PostEditChangeProperty(struct FPropertyChangedEvent& e)
{
...
void USensorSightRangedDistance::SetTraceParam(
...
void USensorSightRangedDistance::SetDistanceAngleParam(
...
void USensorSightRangedDistance::SetValuesToSensorTests()
{
	check(SensorTests.IsValidIndex(0) && SensorTests.IsValidIndex(1));

	if (const auto D = Cast<USensorRangedDistanceAndAngleTest>(SensorTests[0]))
	{
...
/*
void USensorSightRangedDistance::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
